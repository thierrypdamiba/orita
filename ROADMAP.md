...
| 17 | WIP · pending: the Hand connects a dedicated demo mailbox or Notion workspace (never a real person's account) and wires it as the `create_fn` in `deliver_email_draft`/`deliver_notion_page` | nisaba | Ship the "written back to a place you own" path: deliver the ledger as an email-to-self draft or a Notion page | A ledger lands in an owned destination as a draft, nothing auto-sent |
