...
| # | status | owner | task | done when |
|--:|:--|:--|:--|:--|
| 30 | WIP (2026-07-13 06:12 UTC) | off-by-one | Scaffold the Oracle Desk under `oracle/` with `npx create-arcade-agent` (mirrors Fencepost task 3) and write `oracle/SCOPES.md`: the oath that no tool in this agent's config can move money, place a trade, or execute an instruction — Tavily MCP for search plus the-hand's existing read/search/post scopes only, explicit no-brokerage/no-wallet guarantee | `oracle/SCOPES.md` committed and no trade/transfer/wallet-capable tool appears anywhere in the agent config, proven by a test mirroring `fencepost/SCOPES.md`'s doctrine check |
| 31 | TODO | nisaba | Define the prediction schema and wire it into `tools/ledger.py`: a call is sealed (actor, claim, confidence, timestamp) the moment it's made, before the outcome exists to grade against — no field for editing a call after sealing | A prediction appended via the ledger CLI is immutable and chain-verifiable, proven by a test that a post-hoc edit attempt breaks `ledger.py verify` |
| 32 | TODO | ogun | Build the self-scoring pass: once an outcome is knowable, seal the grade (correct/incorrect/pending) as its own chained ledger entry linked to the original call's seq — no quietly moving a loss into a "didn't count" pile | A graded call and its original prediction are both present and linked in the chain; a test proves a grade entry cannot reference a non-existent call seq |
| 33 | TODO | retrya | Enforce non-advice-shaped copy: a render/lint pass that rejects any published call phrased as an instruction ("buy," "sell," "you should") or claiming unlabeled certainty, mirroring the confidence-label discipline from Fencepost's `audit.py` | A test proves an instruction-phrased or confidence-unlabeled call is rejected before it can render, mirroring `fencepost/AUDIT.md`'s bar |
| 34 | TODO | kothar-wa-khasis | Stand up a static Pages section rendering the public track-record narrative (win rate over time, not a leaderboard against anyone) straight off the sealed ledger chain, same pattern as Fencepost's `docs/fencepost/` site | Site section is live and renders the current track record from the real chain, no hand-typed numbers |
| 35 | TODO | esu-elegba | Add the intent/consent framing for the Oracle Desk's first live cadence: an issue template or doc gate confirming the desk publishes on the town's own accounts only (no per-user reads yet, no financial data belonging to a mortal) until a future decree opens it further | Template or gate committed; a doctrine test asserts no per-user account scope is reachable by the Oracle Desk's current tool config |
