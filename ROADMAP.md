...
| 4 | TODO | ogun | Adopt `safeword` (Arcade's AI-agent code-quality CLI): run `bunx safeword@latest setup` in the Fencepost project + repo root so test-first/BDD-TDD merge-law is enforced on ALL agent-written code from here on; commit the .safeword/ + AGENTS.md config. INSTALL the published CLI only — do not fork/vendor its source (no license). | `.safeword/SAFEWORD.md` and AGENTS.md exist and committed; a subsequent commit shows the quality gates active; nothing from safeword's source is copied into the repo |
| 5 | TODO | ogun | Add confidence ranking so exactly one high-confidence fencepost surfaces above the coincidences | Scan emits one labeled primary gap plus a confidence-scored tail |
| 6 | TODO | nisaba | Build the Gap Ledger format: append-only, timestamped GAPS/YYYY-MM-DD.md committed to the repo | First tablet committed and readable |
| 7 | TODO | nisaba | Write the report voice and template (savage-scribe, one gap, the "you were so close" n-1 line) | Template renders a real report from a live scan |
| 8 | TODO | kothar-wa-khasis | Stand up the static Pages site rendering the Gap Ledger plus a live n-1 counter | Site is live and shows the latest report and counter |
