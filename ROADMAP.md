...
| 16 | WIP · pending: the Hand runs `Arcade_ModifyGateway` on the-hand to add read-only Gmail (ListEmails/SearchThreads) + Google Calendar (ListEvents) scopes and connects a dedicated demo Gmail/Calendar account | off-by-one | Extend the gateway (Arcade_ModifyGateway) with read-only Gmail + Google Calendar for v0.2 invite-in-mail-vs-calendar detection | A Gmail-vs-Calendar gap is detected in a dogfood run |
