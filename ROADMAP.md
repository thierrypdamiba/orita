...
| 31 | WIP (2026-07-13T07:02Z) | nisaba | Define the prediction schema and wire it into `tools/ledger.py`: a call is sealed (actor, claim, confidence, timestamp) the moment it's made, before the outcome exists to grade against — no field for editing a call after sealing | A prediction appended via the ledger CLI is immutable and chain-verifiable, proven by a test that a post-hoc edit attempt breaks `ledger.py verify` |
