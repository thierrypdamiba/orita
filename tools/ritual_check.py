...
def _tithe_check() -> ModuleType:
    return _load_once("_ritual_tithe_check", os.path.join(ROOT, "tools", "tithe_check.py"))
...
def check_tithe(
    ...
    houses_dir: str | None = None,
    chronicle_dir: str | None = None,
) -> dict[str, object]:
    """Task 673 (widened tasks 678, 680): fold tithe_check.py's own
    BUILDLOG.md + ROADMAP.md + houses/*/journal/*.md + chronicle/*.md
    scan (every hand-typed "Tithe ... roll N" claim, checked against the
    ratified 0.03 floor test_the_tithe itself enforces) into the one
    block. Unconditional, local-filesystem-only (reads the checkout
    already on disk, no network) -- the same cheap class `check_checkout`/
    `check_vault_leak`/`check_duplicate_regex`/`check_duplicate_function`
    already hold. Never edits anything; a real violation, if one is ever
    found, is a god-on-duty escalation (a transcription error in an
    already-pushed line, or a real doctrine breach), not something this
    check silently repairs."""
    mod = _tithe_check()
    kwargs = {}
...
def run_ritual_check(
    ...
    tithe_houses_dir: str | None = None,
    tithe_chronicle_dir: str | None = None,
    ...
    tithe = check_tithe(
        buildlog_path=tithe_buildlog_path,
        roadmap_path=tithe_roadmap_path,
        houses_dir=tithe_houses_dir,
        chronicle_dir=tithe_chronicle_dir,
    )
...
def format_ritual_check(result: dict[str, Any]) -> str:
    ...
    if tithe_result["clean"]:
        lines.append("  tithe: clean (every hand-typed BUILDLOG.md/ROADMAP.md/houses-journal/chronicle Tithe roll clears the 0.03 floor it claims)")
    else:
