...
swept them at task 163; twenty-nine carry it today -- eleven more `tools/
...
`site_recipe_check.py`, `duplicate_function_check.py`) have independently
repeated the same "no network" claim since, each caught by this module's
own live discovery with no docstring edit required -- which is the whole
point of never hand-typing the list below.
...
now-twenty-nine files import a network-capable module). None of them has ever
...
(eighteen at task 163, twenty-nine today) that could itself go stale --
...
`importlib.util` (everything these twenty-nine files actually use today)
