...
def _last_metrics_entry(metrics_path: str) -> dict | None:
    ...
    is itself a JSON object; a truncated/malformed trailing line, or one
    that parses cleanly but isn't a dict, is skipped, not fatal -- the same
    discipline `connected_users_check.py`'s own reader holds (task 412,
    itself following tasks 306/328). `None` if no reading has ever
