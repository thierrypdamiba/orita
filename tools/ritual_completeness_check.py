...
unwatched the way the Book of the Gate line did. **Updated to 77** the
same hour task 1299's `check_render_one_action_invariant` was wired in
...
EXEMPT_TOOL_FILES = {
    ...
    "scan_files.py": "shared public-file walk + per-path memoization factory library (task 513), imported directly by the five tools/*.py checks that use it (no_grading_check.py, hand_lore_check.py, star_covenant_check.py, arcade_hero_check.py, rider_check.py -- all already wired) rather than loaded standalone by ritual_check.py, the same shape metrics_reader.py/iso_time.py/jsonl_append.py/consent_grant_log.py/text_patterns.py already hold",
    "jsonl_read.py": "shared read-a-JSONL-log library (task 540), imported directly by the fourteen tools/*.py files that use it (arcade_app_watch.py, change_gate.py, child_work_check.py, ci_watch.py, consent_grant_log.py, gateway_toolset_check.py, github_stars_check.py, ledger.py, scribe_growth_check.py, square_check.py, voice_window_check.py, word_watch.py, x_outage_tracker.py, x_post_queue.py -- all already wired or already exempt) rather than loaded standalone by ritual_check.py, the same shape jsonl_append.py/metrics_reader.py/iso_time.py/scan_files.py already hold",
    "violation_format.py": "shared format_violations() renderer library (task 546), imported directly by the six tools/*.py checks that use it (petition_limits_check.py, no_grading_check.py, hand_lore_check.py, star_covenant_check.py, arcade_hero_check.py, rider_check.py -- all already wired) rather than loaded standalone by ritual_check.py, the same shape jsonl_read.py/jsonl_append.py/metrics_reader.py/iso_time.py/scan_files.py already hold",
    "quoted_citation.py": "shared is_quoted_citation() guard library (task 548), imported directly by the five tools/*.py checks that use it (no_grading_check.py, star_covenant_check.py, arcade_hero_check.py, hand_lore_check.py, rider_check.py -- all already wired) rather than loaded standalone by ritual_check.py, the same shape violation_format.py/jsonl_read.py/scan_files.py already hold",
    "sentence_negation.py": "shared iter_sentences()/is_negated_prefix() library (task 548), imported directly by the two tools/*.py checks that use it (hand_lore_check.py, rider_check.py -- both already wired) rather than loaded standalone by ritual_check.py, the same shape quoted_citation.py/violation_format.py already hold",
    "adoption_metric_format.py": "shared format_adoption_result() renderer library (task 551), imported directly by the two tools/*.py checks that use it (connected_users_check.py, toolkits_in_use_check.py -- both already wired) rather than loaded standalone by ritual_check.py, the same shape violation_format.py/quoted_citation.py/sentence_negation.py already hold",
    "date_cadence.py": "shared compute_date_streak_and_gaps() cadence algorithm library (task 555), imported directly by the two tools/*.py checks that use it (metrics_cadence_check.py, report_cadence_check.py -- both already wired) rather than loaded standalone by ritual_check.py, the same shape adoption_metric_format.py/violation_format.py/quoted_citation.py/sentence_negation.py already hold",
}
