...
def load_snapshots(path: str = DEFAULT_SNAPSHOT_PATH) -> list[dict]:
    ...
    tools/ledger.py's _entries() established (task 238) and tasks 239-272
    ...
    rather than sailing through unmarked (tasks 329-351)."""
...
def _reject_malformed(snapshots: list[dict], caller: str) -> None:
    """Raise TweetCadenceTamperedError if any snapshot line came back
    ...
    for s in snapshots:
        if s.get("_malformed"):
            raise TweetCadenceTamperedError(
