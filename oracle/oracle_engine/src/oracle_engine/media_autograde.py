"""Automated grading for media-cadence predictions. (ROADMAP #45, mirrors
#38/#39/#40/#41/#42/#43/#44)

`star_autograde.py`/`fork_autograde.py`/`issue_autograde.py`/
`release_autograde.py`/`follower_autograde.py`/`tweet_autograde.py`/
`listed_autograde.py` closed the loop for their own cadence's claims: find
due calls, re-derive reality over the call's own window, seal once. This
module is the identical loop for `media_cadence.py`'s claims, scored
against the recorded media-snapshot log instead.

A due media-cadence call whose target has no snapshot at or after it yet
...
from oracle_engine.media_cadence import (
    DEFAULT_SNAPSHOT_PATH,
    media_count_at_or_after,
    load_snapshots,
)
...
_MEDIA_CLAIM_RE = re.compile(
    ...
    r"media count will be at least (?P<threshold>\d+) "
)


class MediaAutogradeError(ValueError):
    """A due media-cadence call could not be parsed or scored. Raised
    before any seal -- a malformed or not-yet-scoreable claim is skipped,
    never guessed at."""


def parse_media_claim(claim: str) -> tuple[datetime.datetime, int]:
    """Pull the target timestamp and threshold back out of a claim built by
    `media_cadence.build_prediction`. Raises `MediaAutogradeError` if
    `claim` is not shaped like a media-cadence claim at all -- this module
    only ever grades the kind of call it knows how to re-derive."""
    m = _MEDIA_CLAIM_RE.match(claim)
    if not m:
        raise MediaAutogradeError(f"not a media-cadence-shaped claim: {claim!r}")
...
def _load_claim_payload(detail: str) -> dict[str, object]:
    ...
    return grading.load_claim_payload(detail, MediaAutogradeError)


def find_due_calls(entries: list[dict[str, object]], now: datetime.datetime) -> list[dict[str, object]]:
    """Every `predict` entry, media-cadence-shaped, whose target has
    already passed and that carries no terminal grade yet. Skips (never
    raises on) entries that aren't media-cadence-shaped, and skips (never
    ...
    for entry in entries:
        ...
        try:
            ...
            target, _ = parse_media_claim(cast(str, payload["claim"]))
        except (MediaAutogradeError, KeyError, ValueError, json.JSONDecodeError):
            continue
        if target > now:
            continue
...
def score_call(entry: dict[str, object], snapshots: list[dict[str, object]]) -> str:
    ...
    Raises `MediaAutogradeError` if no snapshot at or after the target has
    ...
    guess."""
    payload = _load_claim_payload(cast(str, entry["detail"]))
    target, threshold = parse_media_claim(cast(str, payload["claim"]))
    actual = media_count_at_or_after(snapshots, target)
    if actual is None:
        raise MediaAutogradeError(
...
def autograde_due_predictions(
    ...
    """Grade every due, ungraded, scoreable media-cadence prediction on the
    ...
    if now.tzinfo is None:
        raise MediaAutogradeError("now must be timezone-aware")
...
if __name__ == "__main__":
    ...
    if not _sealed:
        print("no due, scoreable media-cadence predictions -- quiet run, nothing sealed")
    else:
        for _g in _sealed:
            print(_g["hash"])
