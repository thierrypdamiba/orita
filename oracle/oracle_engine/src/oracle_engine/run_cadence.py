...
class RunCadenceTamperedError(RuntimeError):
    ...
    /PrCadenceTamperedError/ReleaseCadenceTamperedError (tasks
    250-267, valid-JSON-non-dict guard tasks 329-346): both lookup
    functions walk EVERY snapshot looking for the closest one
...
def fetch_run_count(repo: str = DEFAULT_REPO, http_get: Callable[[str], Any] | None = None) -> int:
...
def record_snapshot(count: int, ts: str, path: str = DEFAULT_SNAPSHOT_PATH) -> dict[str, object]:
    ...
    default path and `RunCadenceError`, delegates the actual
    validate-and-write to the one shared implementation."""
    return time_utils.record_snapshot(count, ts, path, error_cls=RunCadenceError)
...
def _reject_malformed(snapshots: list[dict[str, object]], caller: str) -> None:
    """Raise RunCadenceTamperedError if any snapshot line came back marked
    ...
    time_utils.reject_malformed(snapshots, caller, error_cls=RunCadenceTamperedError)


def run_count_at_or_before(snapshots: list[dict[str, object]], when: datetime.datetime) -> int | None:
...
def run_count_at_or_after(snapshots: list[dict[str, object]], when: datetime.datetime) -> int | None:
    ...
    same shape as `run_count_at_or_before` above."""
...
def seal_run_prediction(
    ...
    **build_kwargs: object,
    ...
    """Build one run-cadence prediction and seal it. Thin wrapper around
