"""Automated grading for milestone-cadence predictions. (ROADMAP #67,
mirrors #47/#48/#49)
...
once. This module is the identical loop for `milestone_cadence.py`'s
claims, scored against the recorded milestone-snapshot log instead.

A due milestone-cadence call whose target has no snapshot at or after it
...
from oracle_engine.milestone_cadence import (
    DEFAULT_SNAPSHOT_PATH,
    load_snapshots,
    milestone_count_at_or_after,
)
...
_MILESTONE_CLAIM_RE = re.compile(
    ...
    r"milestone count \(open and closed\) will be at least (?P<threshold>\d+) "
)


class MilestoneAutogradeError(ValueError):
    """A due milestone-cadence call could not be parsed or scored. Raised
    before any seal — a malformed or not-yet-scoreable claim is skipped,
    never guessed at."""


def parse_milestone_claim(claim: str) -> tuple[datetime.datetime, int]:
    """Pull the target timestamp and threshold back out of a claim built by
    `milestone_cadence.build_prediction`. Raises `MilestoneAutogradeError`
    if `claim` is not shaped like a milestone-cadence claim at all — this
    module only ever grades the kind of call it knows how to re-derive."""
    m = _MILESTONE_CLAIM_RE.match(claim)
    if not m:
        raise MilestoneAutogradeError(f"not a milestone-cadence-shaped claim: {claim!r}")
...
def _load_claim_payload(detail: str) -> dict[str, object]:
    ...
    return grading.load_claim_payload(detail, MilestoneAutogradeError)


def find_due_calls(entries: list[dict[str, object]], now: datetime.datetime) -> list[dict[str, object]]:
    """Every `predict` entry, milestone-cadence-shaped, whose target has
    already passed and that carries no terminal grade yet. Skips (never
    raises on) entries that aren't milestone-cadence-shaped, and skips
    ...
    for entry in entries:
        ...
        try:
            ...
            target, _ = parse_milestone_claim(cast(str, payload["claim"]))
        except (MilestoneAutogradeError, KeyError, ValueError, json.JSONDecodeError):
            continue
        if target > now:
            continue
...
def score_call(entry: dict[str, object], snapshots: list[dict[str, object]]) -> str:
    ...
    Raises `MilestoneAutogradeError` if no snapshot at or after the target
    ...
    guess."""
    payload = _load_claim_payload(cast(str, entry["detail"]))
    target, threshold = parse_milestone_claim(cast(str, payload["claim"]))
    actual = milestone_count_at_or_after(snapshots, target)
    if actual is None:
        raise MilestoneAutogradeError(
...
def autograde_due_predictions(
    ...
    """Grade every due, ungraded, scoreable milestone-cadence prediction on
    ...
    if now.tzinfo is None:
        raise MilestoneAutogradeError("now must be timezone-aware")
    ...
    for entry in due:
        ...
        except MilestoneAutogradeError:
            continue
        sealed.append(
            grading.seal_grade(
                actor=actor,
...
if __name__ == "__main__":
    ...
    if not _sealed:
        print("no due, scoreable milestone-cadence predictions — quiet run, nothing sealed")
    else:
        for _g in _sealed:
            print(_g["hash"])
