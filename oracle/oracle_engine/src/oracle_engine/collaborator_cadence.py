...
def fetch_collaborator_count(repo: str = DEFAULT_REPO, http_get: Callable[[str], Any] | None = None) -> int:
...
def record_snapshot(count: int, ts: str, path: str = DEFAULT_SNAPSHOT_PATH) -> dict[str, object]:
    ...
    default path and `CollaboratorCadenceError`, delegates the actual
    validate-and-write to the one shared implementation."""
    return time_utils.record_snapshot(count, ts, path, error_cls=CollaboratorCadenceError)
...
def collaborator_count_at_or_before(snapshots: list[dict[str, object]], when: datetime.datetime) -> int | None:
...
def collaborator_count_at_or_after(snapshots: list[dict[str, object]], when: datetime.datetime) -> int | None:
    """The EARLIEST recorded count at or after `when`; `None` if no
    snapshot that late has landed yet. The grading-side counterpart to
    `collaborator_count_at_or_before`: once a call's window closes, the honest
    ...
    same shape as `collaborator_count_at_or_before` above."""
...
def seal_collaborator_prediction(
    ...
    **build_kwargs: object,
    ...
    """Build one collaborator-cadence prediction and seal it. Thin wrapper around
