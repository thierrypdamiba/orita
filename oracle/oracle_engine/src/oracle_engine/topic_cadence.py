...
def fetch_topic_count(repo: str = DEFAULT_REPO, http_get: Callable[[str], Any] | None = None) -> int:
...
def record_snapshot(count: int, ts: str, path: str = DEFAULT_SNAPSHOT_PATH) -> dict[str, object]:
    ...
    default path and `TopicCadenceError`, delegates the actual
    validate-and-write to the one shared implementation."""
    return time_utils.record_snapshot(count, ts, path, error_cls=TopicCadenceError)
...
def _reject_malformed(snapshots: list[dict[str, object]], caller: str) -> None:
    """Raise TopicCadenceTamperedError if any snapshot line came back marked
    ...
    time_utils.reject_malformed(snapshots, caller, error_cls=TopicCadenceTamperedError)


def topic_count_at_or_before(snapshots: list[dict[str, object]], when: datetime.datetime) -> int | None:
...
def topic_count_at_or_after(snapshots: list[dict[str, object]], when: datetime.datetime) -> int | None:
...
def seal_topic_prediction(
    ...
    **build_kwargs: object,
    ...
    """Build one topic-cadence prediction and seal it. Thin wrapper around
