"""Automated grading for release-cadence predictions. (ROADMAP #41, mirrors #38/#39/#40)

`star_autograde.py`/`fork_autograde.py`/`issue_autograde.py` closed the
loop for their own cadence's claims: find due calls, re-derive reality
over the call's own window, seal once. This module is the identical loop
for `release_cadence.py`'s claims, scored against the recorded
release-snapshot log instead.

A due release-cadence call whose target has no snapshot at or after it
...
from oracle_engine.release_cadence import (
    DEFAULT_SNAPSHOT_PATH,
    load_snapshots,
    release_count_at_or_after,
)
...
_RELEASE_CLAIM_RE = re.compile(
    ...
    r"release count will be at least (?P<threshold>\d+) "
)


class ReleaseAutogradeError(ValueError):
    """A due release-cadence call could not be parsed or scored. Raised
    before any seal — a malformed or not-yet-scoreable claim is skipped,
    never guessed at."""


def parse_release_claim(claim: str) -> tuple[datetime.datetime, int]:
    """Pull the target timestamp and threshold back out of a claim built by
    `release_cadence.build_prediction`. Raises `ReleaseAutogradeError` if
    `claim` is not shaped like a release-cadence claim at all — this
    module only ever grades the kind of call it knows how to re-derive."""
    m = _RELEASE_CLAIM_RE.match(claim)
    if not m:
        raise ReleaseAutogradeError(f"not a release-cadence-shaped claim: {claim!r}")
...
def find_due_calls(entries: list[dict[str, object]], now: datetime.datetime) -> list[dict[str, object]]:
    """Every `predict` entry, release-cadence-shaped, whose target has
    already passed and that carries no terminal grade yet. Skips (never
    raises on) entries that aren't release-cadence-shaped, and skips
    ...
    for entry in entries:
        ...
        try:
            ...
            target, _ = parse_release_claim(cast(str, payload["claim"]))
        except (ReleaseAutogradeError, KeyError, ValueError, json.JSONDecodeError):
            continue
        if target > now:
            continue
...
def score_call(entry: dict[str, object], snapshots: list[dict[str, object]]) -> str:
    ...
    Raises `ReleaseAutogradeError` if no snapshot at or after the target
    ...
    guess."""
    payload = _load_claim_payload(cast(str, entry["detail"]))
    target, threshold = parse_release_claim(cast(str, payload["claim"]))
    actual = release_count_at_or_after(snapshots, target)
    if actual is None:
        raise ReleaseAutogradeError(
...
def autograde_due_predictions(
    ...
    """Grade every due, ungraded, scoreable release-cadence prediction on
    ...
    if now.tzinfo is None:
        raise ReleaseAutogradeError("now must be timezone-aware")
...
if __name__ == "__main__":
    ...
    if not _sealed:
        print("no due, scoreable release-cadence predictions — quiet run, nothing sealed")
    else:
        for _g in _sealed:
            print(_g["hash"])
