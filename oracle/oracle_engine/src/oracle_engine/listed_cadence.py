"""The Oracle Desk's eighth real cadence: a checkable claim about the
town's own public X (`@oritatown`) listed count. (ROADMAP #44)

Tasks 42-43 both read `X_WhoAmI`'s `public_metrics` payload — followers
(task 42, who is watching) and tweet count (task 43, the town's own
output). That same payload carries a third field this desk has never
sealed a claim about: `listed_count` — how many mortal-curated X lists
include `@oritatown`. A follow costs a stranger one tap; a list add means
someone organized the town into their own view of the world on purpose.
That is a genuinely different claim from either prior X cadence: not raw
reach (followers) and not the town's own restraint (tweet count), but
whether anyone curates it. Same platform, same already-cleared `WhoAmI`
allow-list entry, zero new scope.

This module is `tweet_cadence.py` with one field swapped, on purpose — the
same dependency-injection getter shape (no bare-HTTP default exists for a
profile's listed count either, so `_default_whoami_get` raises loudly the
same way `follower_cadence`'s and `tweet_cadence`'s do), the same snapshot
...
signal itself diverges: the claim text names "public listed count," not
"follower count" or "tweet count."
"""
...
DEFAULT_SNAPSHOT_PATH = os.path.normpath(os.path.join(_ORACLE_ROOT, "listed_snapshots.jsonl"))

DEFAULT_HANDLE = "@oritatown"
DEFAULT_HORIZON_HOURS = 168  # a week — being added to a list is rarer than a follow, same cadence as followers
DEFAULT_CONFIDENCE = 0.55
DEFAULT_ACTOR = "kwaku-ananse"


class ListedCadenceError(ValueError):
    """The listed-cadence read or the prediction it produced is not
    well-formed."""


class ListedCadenceTamperedError(RuntimeError):
    """Raised by listed_count_at_or_before/listed_count_at_or_after when the
    ...
    /label_cadence.py's BranchCadenceTamperedError/CollaboratorCadenceTamperedError
    ...
    /LabelCadenceTamperedError (tasks 250-262): both lookup functions walk
...
def _default_whoami_get() -> dict[str, object]:
    ...
    REST endpoint — X exposes no such endpoint for a profile's listed
    count either, same gap `follower_cadence.py`/`tweet_cadence.py`
    document for their own fields. In every real run (live CI included)
    the caller supplies `x_whoami_get=` wired to the-hand's `X_WhoAmI`
    tool (already on `oracle/SCOPES.md`'s `WhoAmI` allow-list). This
    default exists only so an accidental unwired call fails loudly
    instead of silently guessing a count."""
    raise NotImplementedError(
        "listed_cadence has no default X getter — pass x_whoami_get= wired "
        "to the-hand's X_WhoAmI tool (oracle/SCOPES.md's WhoAmI allow-list)"
    )


def fetch_listed_count(x_whoami_get: Callable[[], dict[str, object]] | None = None) -> int:
    """`@oritatown`'s own `public_metrics.listed_count`, off the-hand's
    ...
    if not isinstance(metrics, dict) or "listed_count" not in metrics:
        raise ListedCadenceError(f"malformed X WhoAmI response: {payload!r}")
    return int(metrics["listed_count"])
...
def record_snapshot(count: int, ts: str, path: str = DEFAULT_SNAPSHOT_PATH) -> dict[str, object]:
    ...
    default path and `ListedCadenceError`, delegates the actual
    validate-and-write to the one shared implementation."""
    return time_utils.record_snapshot(count, ts, path, error_cls=ListedCadenceError)
...
def _reject_malformed(snapshots: list[dict[str, object]], caller: str) -> None:
    """Raise ListedCadenceTamperedError if any snapshot line came back marked
    ...
    time_utils.reject_malformed(snapshots, caller, error_cls=ListedCadenceTamperedError)


def listed_count_at_or_before(snapshots: list[dict[str, object]], when: datetime.datetime) -> int | None:
    ...
    (task 578)."""
    _reject_malformed(snapshots, "listed_count_at_or_before")
    return time_utils.count_at_or_before(snapshots, when)


def listed_count_at_or_after(snapshots: list[dict[str, object]], when: datetime.datetime) -> int | None:
    """The EARLIEST recorded count at or after `when`; `None` if no
    snapshot that late has landed yet. The grading-side counterpart to
    `listed_count_at_or_before`: once a call's window closes, the honest
    ...
    same shape as `listed_count_at_or_before` above."""
    _reject_malformed(snapshots, "listed_count_at_or_after")
...
def build_prediction(
    ...
    list curation, plus the confidence sealed alongside it. Pure: reads
    `snapshots`/`now`/`current_count`, writes nothing, decides nothing
    about whether to seal it."""
    if now.tzinfo is None:
        raise ListedCadenceError("now must be timezone-aware")
    now = now.astimezone(datetime.timezone.utc)
    if isinstance(current_count, bool) or not isinstance(current_count, int) or current_count < 0:
        raise ListedCadenceError("current_count must be a non-negative integer")
    if horizon_hours <= 0:
        raise ListedCadenceError(
    ...
    baseline = listed_count_at_or_before(snapshots, baseline_when)
...
def seal_listed_prediction(
    ...
    **build_kwargs: object,
    ...
    """Build one listed-cadence prediction and seal it. Thin wrapper around
...
if __name__ == "__main__":
    ...
    _count = fetch_listed_count()
    record_snapshot(_count, _ts)
    _entry = seal_listed_prediction(now=_now, ts=_ts, current_count=_count)
    print(_entry["hash"])
