...
def fetch_issue_comment_count(repo: str = DEFAULT_REPO, http_get: Callable[[str], Any] | None = None) -> int:
...
def issue_comment_count_at_or_before(snapshots: list[dict[str, object]], when: datetime.datetime) -> int | None:
...
def issue_comment_count_at_or_after(snapshots: list[dict[str, object]], when: datetime.datetime) -> int | None:
