...
def load_snapshots(path: str = DEFAULT_SNAPSHOT_PATH) -> list[dict]:
    ...
    tools/ledger.py's _entries() established (task 238) and tasks 239-260
...
def _reject_malformed(snapshots: list[dict], caller: str) -> None:
    """Raise IssueCommentCadenceTamperedError if any snapshot line came
    ...
    for s in snapshots:
        if s.get("_malformed"):
            raise IssueCommentCadenceTamperedError(
