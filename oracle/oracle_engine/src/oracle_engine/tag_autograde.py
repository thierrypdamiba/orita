"""Automated grading for tag-cadence predictions. (ROADMAP #51, mirrors
#47/#48/#49/#50)
...
once. This module is the identical loop for `tag_cadence.py`'s claims,
scored against the recorded tag-snapshot log instead.

A due tag-cadence call whose target has no snapshot at or after it yet is
...
from oracle_engine.tag_cadence import (
    DEFAULT_SNAPSHOT_PATH,
    load_snapshots,
    tag_count_at_or_after,
)
...
_TAG_CLAIM_RE = re.compile(
    ...
    r"tag count will be at least (?P<threshold>\d+) "
)


class TagAutogradeError(ValueError):
    """A due tag-cadence call could not be parsed or scored. Raised before
    any seal — a malformed or not-yet-scoreable claim is skipped, never
    guessed at."""


def parse_tag_claim(claim: str) -> tuple[datetime.datetime, int]:
    """Pull the target timestamp and threshold back out of a claim built by
    `tag_cadence.build_prediction`. Raises `TagAutogradeError` if `claim`
    is not shaped like a tag-cadence claim at all — this module only ever
    grades the kind of call it knows how to re-derive."""
    m = _TAG_CLAIM_RE.match(claim)
    if not m:
        raise TagAutogradeError(f"not a tag-cadence-shaped claim: {claim!r}")
...
def _load_claim_payload(detail: str) -> dict[str, object]:
    ...
    return grading.load_claim_payload(detail, TagAutogradeError)


def find_due_calls(entries: list[dict[str, object]], now: datetime.datetime) -> list[dict[str, object]]:
    """Every `predict` entry, tag-cadence-shaped, whose target has already
    passed and that carries no terminal grade yet. Skips (never raises on)
    entries that aren't tag-cadence-shaped, and skips (never raises on) a
    ...
    for entry in entries:
        ...
        try:
            ...
            target, _ = parse_tag_claim(cast(str, payload["claim"]))
        except (TagAutogradeError, KeyError, ValueError, json.JSONDecodeError):
            continue
        if target > now:
            continue
...
def score_call(entry: dict[str, object], snapshots: list[dict[str, object]]) -> str:
    ...
    Raises `TagAutogradeError` if no snapshot at or after the target has
    ...
    guess."""
    payload = _load_claim_payload(cast(str, entry["detail"]))
    target, threshold = parse_tag_claim(cast(str, payload["claim"]))
    actual = tag_count_at_or_after(snapshots, target)
    if actual is None:
        raise TagAutogradeError(
...
def autograde_due_predictions(
    ...
    """Grade every due, ungraded, scoreable tag-cadence prediction on the
    ...
    if now.tzinfo is None:
        raise TagAutogradeError("now must be timezone-aware")
...
if __name__ == "__main__":
    ...
    if not _sealed:
        print("no due, scoreable tag-cadence predictions — quiet run, nothing sealed")
    else:
        for _g in _sealed:
            print(_g["hash"])
