...
def fetch_deployment_count(repo: str = DEFAULT_REPO, http_get: Callable[[str], Any] | None = None) -> int:
...
def deployment_count_at_or_before(snapshots: list[dict[str, object]], when: datetime.datetime) -> int | None:
...
def deployment_count_at_or_after(snapshots: list[dict[str, object]], when: datetime.datetime) -> int | None:
