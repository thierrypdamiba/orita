...
def load_snapshots(path: str = DEFAULT_SNAPSHOT_PATH) -> list[dict]:
    ...
    tools/ledger.py's _entries() established (task 238) and tasks 239-255
...
def record_snapshot(count: int, ts: str, path: str = DEFAULT_SNAPSHOT_PATH) -> dict:
    ...
    if isinstance(count, bool) or not isinstance(count, int) or count < 0:
        raise DeploymentCadenceError("count must be a non-negative integer")
...
def _reject_malformed(snapshots: list[dict], caller: str) -> None:
    """Raise DeploymentCadenceTamperedError if any snapshot line came back
    ...
    for s in snapshots:
        if s.get("_malformed"):
            raise DeploymentCadenceTamperedError(
...
def deployment_count_at_or_after(snapshots: list[dict], when: datetime.datetime) -> int | None:
    ...
    `deployment_count_at_or_before`: once a call's window closes, the
...
def seal_deployment_prediction(
    ...
    **build_kwargs,
    ...
    """Build one deployment-cadence prediction and seal it. `now` and `ts`
