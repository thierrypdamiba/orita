...
def load_snapshots(path: str = DEFAULT_SNAPSHOT_PATH) -> list[dict]:
    ...
    tools/ledger.py's _entries() established (task 238) and tasks 239-251
    ...
    sailing through unmarked, mirroring tasks 328-330's identical guard."""
...
def record_snapshot(count: int, ts: str, path: str = DEFAULT_SNAPSHOT_PATH) -> dict:
    ...
    if isinstance(count, bool) or not isinstance(count, int) or count < 0:
        raise CommentCadenceError("count must be a non-negative integer")
...
def _reject_malformed(snapshots: list[dict], caller: str) -> None:
    """Raise CommentCadenceTamperedError if any snapshot line came back
    ...
    for s in snapshots:
        if s.get("_malformed"):
            raise CommentCadenceTamperedError(
...
def seal_comment_prediction(
    ...
    **build_kwargs,
    ...
    """Build one comment-cadence prediction and seal it. `now` and `ts` are
