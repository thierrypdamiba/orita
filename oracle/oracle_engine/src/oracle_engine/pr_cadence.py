...
class PrCadenceTamperedError(RuntimeError):
    """Raised by pr_count_at_or_before/pr_count_at_or_after when the
    ...
    /milestone_cadence.py's BranchCadenceTamperedError
    ...
    /MilestoneCadenceTamperedError (tasks 250-265): both lookup functions
...
def fetch_open_pr_count(repo: str = DEFAULT_REPO, http_get: Callable[[str], Any] | None = None) -> int:
...
def record_snapshot(count: int, ts: str, path: str = DEFAULT_SNAPSHOT_PATH) -> dict[str, object]:
    ...
    default path and `PrCadenceError`, delegates the actual
    validate-and-write to the one shared implementation."""
    return time_utils.record_snapshot(count, ts, path, error_cls=PrCadenceError)
...
def _reject_malformed(snapshots: list[dict[str, object]], caller: str) -> None:
    """Raise PrCadenceTamperedError if any snapshot line came back marked
    ...
    time_utils.reject_malformed(snapshots, caller, error_cls=PrCadenceTamperedError)


def pr_count_at_or_before(snapshots: list[dict[str, object]], when: datetime.datetime) -> int | None:
    ...
    (task 578)."""
    _reject_malformed(snapshots, "pr_count_at_or_before")
    return time_utils.count_at_or_before(snapshots, when)


def pr_count_at_or_after(snapshots: list[dict[str, object]], when: datetime.datetime) -> int | None:
    ...
    same shape as `pr_count_at_or_before` above."""
    _reject_malformed(snapshots, "pr_count_at_or_after")
...
def seal_pr_prediction(
    ...
    **build_kwargs: object,
    ...
    """Build one pr-cadence prediction and seal it. Thin wrapper around
