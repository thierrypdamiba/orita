...
def fetch_follower_count(x_whoami_get: Callable[[], dict[str, object]] | None = None) -> int:
...
def record_snapshot(count: int, ts: str, path: str = DEFAULT_SNAPSHOT_PATH) -> dict[str, object]:
    ...
    default path and `FollowerCadenceError`, delegates the actual
    validate-and-write to the one shared implementation."""
    return time_utils.record_snapshot(count, ts, path, error_cls=FollowerCadenceError)
...
def _reject_malformed(snapshots: list[dict[str, object]], caller: str) -> None:
    """Raise FollowerCadenceTamperedError if any snapshot line came back marked
    ...
    time_utils.reject_malformed(snapshots, caller, error_cls=FollowerCadenceTamperedError)


def follower_count_at_or_before(snapshots: list[dict[str, object]], when: datetime.datetime) -> int | None:
...
def follower_count_at_or_after(snapshots: list[dict[str, object]], when: datetime.datetime) -> int | None:
    ...
    same shape as `follower_count_at_or_before` above."""
...
def seal_follower_prediction(
    ...
    **build_kwargs: object,
    ...
    """Build one follower-cadence prediction and seal it. Thin wrapper around
