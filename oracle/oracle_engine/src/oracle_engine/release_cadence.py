...
class ReleaseCadenceTamperedError(RuntimeError):
    ...
    250-266): both lookup functions walk EVERY snapshot looking for the
...
def load_snapshots(path: str = DEFAULT_SNAPSHOT_PATH) -> list[dict]:
    ...
    tools/ledger.py's _entries() established (task 238) and tasks 239-266
...
def _reject_malformed(snapshots: list[dict], caller: str) -> None:
    """Raise ReleaseCadenceTamperedError if any snapshot line came back
    ...
    for s in snapshots:
        if s.get("_malformed"):
            raise ReleaseCadenceTamperedError(
