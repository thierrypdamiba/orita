...
def fetch_contributor_count(repo: str = DEFAULT_REPO, http_get=None) -> int:
...
def record_snapshot(count: int, ts: str, path: str = DEFAULT_SNAPSHOT_PATH) -> dict:
    ...
    if isinstance(count, bool) or not isinstance(count, int) or count < 0:
        raise ContributorCadenceError("count must be a non-negative integer")
...
def _reject_malformed(snapshots: list[dict], caller: str) -> None:
    """Raise ContributorCadenceTamperedError if any snapshot line came
    ...
    for s in snapshots:
        if s.get("_malformed"):
            raise ContributorCadenceTamperedError(
...
def contributor_count_at_or_before(snapshots: list[dict], when: datetime.datetime) -> int | None:
...
def contributor_count_at_or_after(snapshots: list[dict], when: datetime.datetime) -> int | None:
    """The EARLIEST recorded count at or after `when`; `None` if no
    snapshot that late has landed yet. The grading-side counterpart to
    `contributor_count_at_or_before`: once a call's window closes, the
...
def seal_contributor_prediction(
    ...
    **build_kwargs,
    ...
    """Build one contributor-cadence prediction and seal it. `now` and `ts`
