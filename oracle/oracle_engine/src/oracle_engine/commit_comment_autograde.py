...
def _load_claim_payload(detail: str) -> dict[str, object]:
    ...
    return grading.load_claim_payload(detail, CommitCommentAutogradeError)
...
def find_due_calls(entries: list[dict[str, object]], now: datetime.datetime) -> list[dict[str, object]]:
    ...
    and skips (never raises on) a prior grade record that is present but
    ...
    for entry in entries:
        ...
        try:
            ...
            target, _ = parse_commit_comment_claim(cast(str, payload["claim"]))
...
def score_call(entry: dict[str, object], snapshots: list[dict[str, object]]) -> str:
    ...
    payload = _load_claim_payload(cast(str, entry["detail"]))
    target, threshold = parse_commit_comment_claim(cast(str, payload["claim"]))
