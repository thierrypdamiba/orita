...
def load_snapshots(path: str = DEFAULT_SNAPSHOT_PATH) -> list[dict]:
    ...
    rather than sailing through unmarked (tasks 329-348)."""
...
def record_snapshot(count: int, ts: str, path: str = DEFAULT_SNAPSHOT_PATH) -> dict:
    ...
    if isinstance(count, bool) or not isinstance(count, int) or count < 0:
        raise SubscriberCadenceError("count must be a non-negative integer")
...
def subscriber_count_at_or_after(snapshots: list[dict], when: datetime.datetime) -> int | None:
    ...
    `subscriber_count_at_or_before`: once a call's window closes, the
...
def seal_subscriber_prediction(
    ...
    **build_kwargs,
    ...
    """Build one subscriber-cadence prediction and seal it. `now` and `ts`
