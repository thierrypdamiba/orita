...
def fetch_label_count(repo: str = DEFAULT_REPO, http_get: Callable[[str], Any] | None = None) -> int:
...
def label_count_at_or_before(snapshots: list[dict[str, object]], when: datetime.datetime) -> int | None:
...
def label_count_at_or_after(snapshots: list[dict[str, object]], when: datetime.datetime) -> int | None:
    ...
    same shape as `label_count_at_or_before` above."""
...
def seal_label_prediction(
    ...
    **build_kwargs: object,
    ...
    """Build one label-cadence prediction and seal it. Thin wrapper around
