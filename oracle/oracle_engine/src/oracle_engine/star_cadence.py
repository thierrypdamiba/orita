...
def fetch_star_count(repo: str = DEFAULT_REPO, http_get: Callable[[str], Any] | None = None) -> int:
...
def star_count_at_or_before(snapshots: list[dict[str, object]], when: datetime.datetime) -> int | None:
...
def star_count_at_or_after(snapshots: list[dict[str, object]], when: datetime.datetime) -> int | None:
    """The EARLIEST recorded count at or after `when`; `None` if no
    snapshot that late has landed yet. The grading-side counterpart to
    `star_count_at_or_before`: once a call's window closes, the honest
    ...
    same shape as `star_count_at_or_before` above."""
...
def seal_star_prediction(
    ...
    **build_kwargs: object,
    ...
    """Build one star-cadence prediction and seal it. Thin wrapper around
