"""Task 50. The Oracle Desk's fourteenth real cadence: one checkable claim
about the town's own public GitHub subscriber (watcher) count, sourced with
no Arcade scope at all (a repo's subscriber list is public data),
copylint-clean, sealed to a real (scratch, in these tests) ledger before its
outcome is knowable.
"""
...
from oracle_engine import copylint, prediction, subscriber_cadence  # noqa: E402
...
class TestFetchSubscriberCount(unittest.TestCase):
    def test_reads_the_list_length_off_a_single_page(self):
        def fake_get(url):
            self.assertEqual(
                url,
                "https://api.github.com/repos/thierrypdamiba/orita/subscribers?per_page=100&page=1",
            )
            return [{"login": "thierrypdamiba"}]

        self.assertEqual(subscriber_cadence.fetch_subscriber_count(http_get=fake_get), 1)
    ...
    def test_pages_until_a_short_page_ends_it(self):
        ...
        def fake_get(url):
            ...
            if page == 1:
                return [{"login": f"u{i}"} for i in range(100)]
            return [{"login": "u100"}, {"login": "u101"}]

        self.assertEqual(subscriber_cadence.fetch_subscriber_count(http_get=fake_get), 102)
        self.assertEqual(len(calls), 2)

    def test_rejects_a_malformed_response(self):
        with self.assertRaises(subscriber_cadence.SubscriberCadenceError):
            subscriber_cadence.fetch_subscriber_count(http_get=lambda url: {"nope": True})

    def test_empty_repo_is_zero(self):
        self.assertEqual(subscriber_cadence.fetch_subscriber_count(http_get=lambda url: []), 0)


class TestSnapshots(unittest.TestCase):
    def test_load_snapshots_of_a_missing_file_is_empty(self):
        self.assertEqual(subscriber_cadence.load_snapshots("/does/not/exist.jsonl"), [])

    def test_record_then_load_round_trips(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = os.path.join(tmp, "snapshots.jsonl")
            subscriber_cadence.record_snapshot(1, "2026-07-13T00:00:00+00:00", path=path)
            subscriber_cadence.record_snapshot(2, "2026-07-14T00:00:00+00:00", path=path)
            snaps = subscriber_cadence.load_snapshots(path)
...
class TestSubscriberCountAtOrBefore(unittest.TestCase):
    def test_returns_none_with_no_early_enough_snapshot(self):
        ...
        self.assertIsNone(subscriber_cadence.subscriber_count_at_or_before(snaps, when))
    ...
    def test_returns_the_latest_at_or_before(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        self.assertEqual(subscriber_cadence.subscriber_count_at_or_before(snaps, when), 1)
    ...
    def test_raises_tampered_error_on_a_malformed_line_instead_of_crashing(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        with self.assertRaises(subscriber_cadence.SubscriberCadenceTamperedError):
            subscriber_cadence.subscriber_count_at_or_before(snaps, when)


class TestSubscriberCountAtOrAfter(unittest.TestCase):
    def test_returns_none_with_no_late_enough_snapshot(self):
        snaps = [{"ts": "2026-07-11T00:00:00+00:00", "count": 1}]
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        self.assertIsNone(subscriber_cadence.subscriber_count_at_or_after(snaps, when))
    ...
    def test_raises_tampered_error_on_a_malformed_line_instead_of_crashing(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        with self.assertRaises(subscriber_cadence.SubscriberCadenceTamperedError):
            subscriber_cadence.subscriber_count_at_or_after(snaps, when)
    ...
    def test_returns_the_earliest_at_or_after(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        self.assertEqual(subscriber_cadence.subscriber_count_at_or_after(snaps, when), 2)


class TestBuildPrediction(unittest.TestCase):
    def test_claim_names_the_threshold_and_target(self):
        payload = subscriber_cadence.build_prediction(_NOW, [], current_count=1, horizon_hours=336)
        self.assertIn("2", payload["claim"])
        self.assertIn("2026-08-03T12:00:00Z", payload["claim"])
        self.assertEqual(payload["confidence"], subscriber_cadence.DEFAULT_CONFIDENCE)
    ...
    def test_a_baseline_snapshot_names_the_real_delta(self):
        ...
        payload = subscriber_cadence.build_prediction(_NOW, snaps, current_count=1, horizon_hours=336)
        self.assertIn("+0", payload["claim"])

    def test_claim_clears_copylint(self):
        payload = subscriber_cadence.build_prediction(_NOW, [], current_count=1)
        result = copylint.enforce_copy(payload["claim"], payload["confidence"])
        self.assertTrue(result.ok)

    def test_rejects_naive_datetime(self):
        with self.assertRaises(subscriber_cadence.SubscriberCadenceError):
            subscriber_cadence.build_prediction(
                datetime.datetime(2026, 7, 20, 12, 0), [], current_count=1
            )

    def test_rejects_a_negative_count(self):
        with self.assertRaises(subscriber_cadence.SubscriberCadenceError):
            subscriber_cadence.build_prediction(_NOW, [], current_count=-1)
    ...
    def test_rejects_zero_horizon_hours(self):
        ...
        # be sealed.
        with self.assertRaises(subscriber_cadence.SubscriberCadenceError):
            subscriber_cadence.build_prediction(_NOW, [], current_count=1, horizon_hours=0)
    ...
    def test_rejects_negative_horizon_hours(self):
        ...
        with self.assertRaises(subscriber_cadence.SubscriberCadenceError):
            subscriber_cadence.build_prediction(_NOW, [], current_count=1, horizon_hours=-24)
    ...
    def test_non_utc_aware_now_still_targets_the_true_utc_instant(self):
        ...
        payload = subscriber_cadence.build_prediction(non_utc_now, [], current_count=1, horizon_hours=336)
        self.assertIn("2026-08-03T12:00:00Z", payload["claim"])
...
class TestSealSubscriberPrediction(unittest.TestCase):
    def test_seals_a_real_predict_entry_to_a_scratch_ledger(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            entry = subscriber_cadence.seal_subscriber_prediction(
                now=_NOW,
    ...
    def test_a_tampered_sealed_subscriber_prediction_breaks_verify(self):
        with tempfile.TemporaryDirectory() as tmp:
            ledger_path = os.path.join(tmp, "ledger.jsonl")
            mod = _fresh_ledger_module(ledger_path)
            subscriber_cadence.seal_subscriber_prediction(
                now=_NOW,
                ts=_NOW.isoformat(timespec="seconds"),
                current_count=1,
                snapshots=[],
