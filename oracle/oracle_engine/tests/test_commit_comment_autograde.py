"""Task 93's grading half: scoring a commit-comment-cadence prediction the
...
from oracle_engine import commit_comment_autograde, commit_comment_cadence, grading, prediction  # noqa: E402
...
class AutogradeTestBase(unittest.TestCase):
    def setUp(self):
        self.ledger_path = os.path.join(_TESTS_DIR, "_scratch_commit_comment_autograde_ledger.jsonl")
        if os.path.exists(self.ledger_path):
            os.remove(self.ledger_path)
        self.mod = _fresh_ledger_module(self.ledger_path)
        self.snapshot_path = os.path.join(_TESTS_DIR, "_scratch_commit_comment_snapshots.jsonl")
    ...
    def _seal_commit_comment_call(self, sealed_at: str, target: str, threshold: int = 1, confidence: float = 0.5):
        claim = (
            f"By {target}, thierrypdamiba/orita's public GitHub commit comment "
...
class TestParseCommitCommentClaim(unittest.TestCase):
    def test_parses_target_and_threshold(self):
        claim = (
            "By 2026-08-03T12:00:00Z, thierrypdamiba/orita's public GitHub commit "
            "comment count will be at least 1 (currently 0, net change over the "
            "past 336h: +0)."
        )
        target, threshold = commit_comment_autograde.parse_commit_comment_claim(claim)
        self.assertEqual(threshold, 1)
        self.assertEqual(
            target, datetime.datetime(2026, 8, 3, 12, 0, tzinfo=datetime.timezone.utc)
        )

    def test_rejects_a_non_commit_comment_claim(self):
        with self.assertRaises(commit_comment_autograde.CommitCommentAutogradeError):
            commit_comment_autograde.parse_commit_comment_claim("a fork opens within 30 days")


class TestFindDueCalls(AutogradeTestBase):
    def test_a_call_whose_window_has_not_closed_is_not_due(self):
        self._seal_commit_comment_call(sealed_at="2026-07-20T12:00:00+00:00", target="2026-08-03T12:00:00Z")
        now = datetime.datetime(2026, 7, 21, tzinfo=datetime.timezone.utc)
        due = commit_comment_autograde.find_due_calls(self.mod._entries(), now)
    ...
    def test_an_already_terminally_graded_call_is_not_due_again(self):
        call = self._seal_commit_comment_call(sealed_at="2026-07-20T12:00:00+00:00", target="2026-08-03T12:00:00Z")
        grading.seal_grade(
...
class TestScoreCall(AutogradeTestBase):
    def test_scores_correct_when_the_recorded_snapshot_meets_threshold(self):
        call = self._seal_commit_comment_call(
            sealed_at="2026-07-20T12:00:00+00:00", target="2026-08-03T12:00:00Z", threshold=1
        )
        commit_comment_cadence.record_snapshot(1, "2026-08-03T13:00:00+00:00", path=self.snapshot_path)
        snapshots = commit_comment_cadence.load_snapshots(self.snapshot_path)
        self.assertEqual(commit_comment_autograde.score_call(call, snapshots), "correct")

    def test_scores_incorrect_when_the_recorded_snapshot_misses_threshold(self):
        call = self._seal_commit_comment_call(
            sealed_at="2026-07-20T12:00:00+00:00", target="2026-08-03T12:00:00Z", threshold=1
        )
        commit_comment_cadence.record_snapshot(0, "2026-08-03T13:00:00+00:00", path=self.snapshot_path)
        snapshots = commit_comment_cadence.load_snapshots(self.snapshot_path)
        self.assertEqual(commit_comment_autograde.score_call(call, snapshots), "incorrect")
    ...
    def test_raises_when_no_snapshot_exists_at_or_before_the_target(self):
        ...
        with self.assertRaises(commit_comment_autograde.CommitCommentAutogradeError):
            commit_comment_autograde.score_call(call, [])
...
class TestAutogradeDuePredictions(AutogradeTestBase):
    def test_a_quiet_run_seals_nothing(self):
        self._seal_commit_comment_call(sealed_at="2026-07-20T12:00:00+00:00", target="2026-08-03T12:00:00Z")
        now = datetime.datetime(2026, 7, 21, tzinfo=datetime.timezone.utc)
        sealed = commit_comment_autograde.autograde_due_predictions(
    ...
    def test_running_twice_does_not_double_grade(self):
        self._seal_commit_comment_call(
            sealed_at="2026-07-20T12:00:00+00:00", target="2026-08-03T12:00:00Z", threshold=1
        )
        commit_comment_cadence.record_snapshot(1, "2026-08-03T13:00:00+00:00", path=self.snapshot_path)
        now = datetime.datetime(2026, 8, 4, tzinfo=datetime.timezone.utc)
        first = commit_comment_autograde.autograde_due_predictions(
            now=now, ts=now.isoformat(), snapshot_path=self.snapshot_path, ledger_module=self.mod,
        )
        second = commit_comment_autograde.autograde_due_predictions(
            now=now, ts=now.isoformat(), snapshot_path=self.snapshot_path, ledger_module=self.mod,
        )
    ...
    def test_now_must_be_timezone_aware(self):
        with self.assertRaises(commit_comment_autograde.CommitCommentAutogradeError):
            commit_comment_autograde.autograde_due_predictions(
