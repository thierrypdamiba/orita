"""Task 53. The Oracle Desk's seventeenth real cadence: one checkable claim
about the town's own public GitHub repository topic count, sourced with no
Arcade scope at all (a repo's topic list is public data off the same
repo-object read tasks 38-40 already exercise), copylint-clean, sealed to a
...
from oracle_engine import copylint, prediction, topic_cadence  # noqa: E402
...
class TestFetchTopicCount(unittest.TestCase):
    def test_reads_the_topics_array_length_off_the_injected_getter(self):
        def fake_get(url):
            self.assertEqual(url, "https://api.github.com/repos/thierrypdamiba/orita")
            return {"topics": ["ai-agents", "arcade", "mcp"]}

        self.assertEqual(topic_cadence.fetch_topic_count(http_get=fake_get), 3)

    def test_empty_topics_is_zero(self):
        self.assertEqual(topic_cadence.fetch_topic_count(http_get=lambda url: {"topics": []}), 0)

    def test_rejects_a_malformed_response(self):
        with self.assertRaises(topic_cadence.TopicCadenceError):
            topic_cadence.fetch_topic_count(http_get=lambda url: {"nope": True})

    def test_rejects_a_non_list_topics_field(self):
        with self.assertRaises(topic_cadence.TopicCadenceError):
            topic_cadence.fetch_topic_count(http_get=lambda url: {"topics": "oops"})


class TestSnapshots(unittest.TestCase):
    def test_load_snapshots_of_a_missing_file_is_empty(self):
        self.assertEqual(topic_cadence.load_snapshots("/does/not/exist.jsonl"), [])

    def test_record_then_load_round_trips(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = os.path.join(tmp, "snapshots.jsonl")
            topic_cadence.record_snapshot(6, "2026-07-13T00:00:00+00:00", path=path)
            topic_cadence.record_snapshot(7, "2026-07-14T00:00:00+00:00", path=path)
            snaps = topic_cadence.load_snapshots(path)
            self.assertEqual([s["count"] for s in snaps], [6, 7])

    def test_record_rejects_a_negative_count(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = os.path.join(tmp, "snapshots.jsonl")
            with self.assertRaises(topic_cadence.TopicCadenceError):
                topic_cadence.record_snapshot(-1, "2026-07-13T00:00:00+00:00", path=path)
...
class TestTopicCountAtOrBefore(unittest.TestCase):
    def test_returns_none_with_no_early_enough_snapshot(self):
        snaps = [{"ts": "2026-07-14T00:00:00+00:00", "count": 6}]
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        self.assertIsNone(topic_cadence.topic_count_at_or_before(snaps, when))

    def test_returns_the_latest_at_or_before(self):
        snaps = [
            {"ts": "2026-07-11T00:00:00+00:00", "count": 5},
            {"ts": "2026-07-12T00:00:00+00:00", "count": 5},
...
class TestTopicCountAtOrAfter(unittest.TestCase):
    ...
    def test_returns_the_earliest_at_or_after(self):
        snaps = [
            {"ts": "2026-07-11T00:00:00+00:00", "count": 5},
            {"ts": "2026-07-14T00:00:00+00:00", "count": 6},
            {"ts": "2026-07-16T00:00:00+00:00", "count": 7},
        ]
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        self.assertEqual(topic_cadence.topic_count_at_or_after(snaps, when), 6)


class TestBuildPrediction(unittest.TestCase):
    def test_claim_names_the_threshold_and_target(self):
        payload = topic_cadence.build_prediction(_NOW, [], current_count=6, horizon_hours=336)
        self.assertIn("7", payload["claim"])
        self.assertIn("2026-08-03T12:00:00Z", payload["claim"])
        self.assertEqual(payload["confidence"], topic_cadence.DEFAULT_CONFIDENCE)
    ...
    def test_a_baseline_snapshot_names_the_real_delta(self):
        ...
        snaps = [{"ts": baseline_when.isoformat(), "count": 6}]
        payload = topic_cadence.build_prediction(_NOW, snaps, current_count=6, horizon_hours=336)
        self.assertIn("+0", payload["claim"])

    def test_claim_clears_copylint(self):
        payload = topic_cadence.build_prediction(_NOW, [], current_count=6)
        result = copylint.enforce_copy(payload["claim"], payload["confidence"])
        self.assertTrue(result.ok)

    def test_rejects_naive_datetime(self):
        with self.assertRaises(topic_cadence.TopicCadenceError):
            topic_cadence.build_prediction(
                datetime.datetime(2026, 7, 20, 12, 0), [], current_count=6
            )

    def test_rejects_a_negative_count(self):
        with self.assertRaises(topic_cadence.TopicCadenceError):
            topic_cadence.build_prediction(_NOW, [], current_count=-1)
    ...
    def test_rejects_zero_horizon_hours(self):
        ...
        # be sealed.
        with self.assertRaises(topic_cadence.TopicCadenceError):
            topic_cadence.build_prediction(_NOW, [], current_count=1, horizon_hours=0)
    ...
    def test_rejects_negative_horizon_hours(self):
        ...
        with self.assertRaises(topic_cadence.TopicCadenceError):
            topic_cadence.build_prediction(_NOW, [], current_count=1, horizon_hours=-24)
    ...
    def test_non_utc_aware_now_still_targets_the_true_utc_instant(self):
        ...
        payload = topic_cadence.build_prediction(non_utc_now, [], current_count=6, horizon_hours=336)
        self.assertIn("2026-08-03T12:00:00Z", payload["claim"])
...
class TestSealTopicPrediction(unittest.TestCase):
    def test_seals_a_real_predict_entry_to_a_scratch_ledger(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            entry = topic_cadence.seal_topic_prediction(
                now=_NOW,
    ...
    def test_a_tampered_sealed_topic_prediction_breaks_verify(self):
        with tempfile.TemporaryDirectory() as tmp:
            ledger_path = os.path.join(tmp, "ledger.jsonl")
            mod = _fresh_ledger_module(ledger_path)
            topic_cadence.seal_topic_prediction(
                now=_NOW,
                ts=_NOW.isoformat(timespec="seconds"),
                current_count=6,
                snapshots=[],
            ...
            tampered["detail"] = tampered["detail"].replace("6", "999")
