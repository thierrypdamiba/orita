"""Task 77. The Oracle Desk's twenty-fourth real cadence: one checkable
claim about the town's own public GitHub issue-and-PR-thread comment
count, sourced with no Arcade scope at all (a repo's issue comments are
public data), copylint-clean, sealed to a real (scratch, in these tests)
ledger before its outcome is knowable.
"""
...
from oracle_engine import copylint, issue_comment_cadence, prediction  # noqa: E402
...
class TestFetchIssueCommentCount(unittest.TestCase):
    def test_reads_the_list_length_off_a_single_page(self):
        def fake_get(url):
            self.assertEqual(
                url,
                "https://api.github.com/repos/thierrypdamiba/orita/issues/comments?per_page=100&page=1",
            )
            return [{"id": 1}]

        self.assertEqual(issue_comment_cadence.fetch_issue_comment_count(http_get=fake_get), 1)
    ...
    def test_pages_until_a_short_page_ends_it(self):
        ...
        self.assertEqual(issue_comment_cadence.fetch_issue_comment_count(http_get=fake_get), 102)
        self.assertEqual(len(calls), 2)

    def test_rejects_a_malformed_response(self):
        with self.assertRaises(issue_comment_cadence.IssueCommentCadenceError):
            issue_comment_cadence.fetch_issue_comment_count(http_get=lambda url: {"nope": True})

    def test_no_comments_yet_is_zero(self):
        self.assertEqual(issue_comment_cadence.fetch_issue_comment_count(http_get=lambda url: []), 0)


class TestSnapshots(unittest.TestCase):
    def test_load_snapshots_of_a_missing_file_is_empty(self):
        self.assertEqual(issue_comment_cadence.load_snapshots("/does/not/exist.jsonl"), [])

    def test_record_then_load_round_trips(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = os.path.join(tmp, "snapshots.jsonl")
            issue_comment_cadence.record_snapshot(12, "2026-07-13T00:00:00+00:00", path=path)
            issue_comment_cadence.record_snapshot(13, "2026-07-14T00:00:00+00:00", path=path)
            snaps = issue_comment_cadence.load_snapshots(path)
            self.assertEqual([s["count"] for s in snaps], [12, 13])

    def test_record_rejects_a_negative_count(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = os.path.join(tmp, "snapshots.jsonl")
            with self.assertRaises(issue_comment_cadence.IssueCommentCadenceError):
                issue_comment_cadence.record_snapshot(-1, "2026-07-13T00:00:00+00:00", path=path)

    def test_never_rewrites_a_prior_line(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = os.path.join(tmp, "snapshots.jsonl")
            issue_comment_cadence.record_snapshot(0, "2026-07-13T00:00:00+00:00", path=path)
            with open(path) as f:
                first_write = f.read()
            issue_comment_cadence.record_snapshot(1, "2026-07-14T00:00:00+00:00", path=path)
            with open(path) as f:
                second_write = f.read()
...
class TestIssueCommentCountAtOrBefore(unittest.TestCase):
    ...
    def test_returns_the_latest_at_or_before(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        self.assertEqual(issue_comment_cadence.issue_comment_count_at_or_before(snaps, when), 0)
...
class TestIssueCommentCountAtOrAfter(unittest.TestCase):
    def test_returns_none_with_no_late_enough_snapshot(self):
        snaps = [{"ts": "2026-07-11T00:00:00+00:00", "count": 0}]
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        self.assertIsNone(issue_comment_cadence.issue_comment_count_at_or_after(snaps, when))
    ...
    def test_returns_the_earliest_at_or_after(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        self.assertEqual(issue_comment_cadence.issue_comment_count_at_or_after(snaps, when), 1)
    ...
    def test_raises_tampered_error_on_a_malformed_line_instead_of_crashing(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        with self.assertRaises(issue_comment_cadence.IssueCommentCadenceTamperedError):
            issue_comment_cadence.issue_comment_count_at_or_after(snaps, when)
...
class TestBuildPrediction(unittest.TestCase):
    def test_claim_names_the_threshold_and_target(self):
        payload = issue_comment_cadence.build_prediction(_NOW, [], current_count=12, horizon_hours=336)
        self.assertIn("13", payload["claim"])
        self.assertIn("2026-08-03T12:00:00Z", payload["claim"])
        self.assertEqual(payload["confidence"], issue_comment_cadence.DEFAULT_CONFIDENCE)

    def test_no_baseline_snapshot_says_so_honestly(self):
        payload = issue_comment_cadence.build_prediction(_NOW, [], current_count=0, horizon_hours=336)
    ...
    def test_a_baseline_snapshot_names_the_real_delta(self):
        ...
        snaps = [{"ts": baseline_when.isoformat(), "count": 10}]
        payload = issue_comment_cadence.build_prediction(_NOW, snaps, current_count=12, horizon_hours=336)
        self.assertIn("+2", payload["claim"])

    def test_claim_clears_copylint(self):
        payload = issue_comment_cadence.build_prediction(_NOW, [], current_count=0)
        result = copylint.enforce_copy(payload["claim"], payload["confidence"])
        self.assertTrue(result.ok)

    def test_rejects_naive_datetime(self):
        with self.assertRaises(issue_comment_cadence.IssueCommentCadenceError):
            issue_comment_cadence.build_prediction(
                datetime.datetime(2026, 7, 20, 12, 0), [], current_count=0
            )

    def test_rejects_a_negative_count(self):
        with self.assertRaises(issue_comment_cadence.IssueCommentCadenceError):
            issue_comment_cadence.build_prediction(_NOW, [], current_count=-1)
    ...
    def test_rejects_zero_horizon_hours(self):
        ...
        # be sealed.
        with self.assertRaises(issue_comment_cadence.IssueCommentCadenceError):
            issue_comment_cadence.build_prediction(_NOW, [], current_count=1, horizon_hours=0)
    ...
    def test_rejects_negative_horizon_hours(self):
        ...
        with self.assertRaises(issue_comment_cadence.IssueCommentCadenceError):
            issue_comment_cadence.build_prediction(_NOW, [], current_count=1, horizon_hours=-24)
    ...
    def test_non_utc_aware_now_still_targets_the_true_utc_instant(self):
        ...
        payload = issue_comment_cadence.build_prediction(
            now_eastern, [], current_count=12, horizon_hours=336
        )
        self.assertIn("2026-08-03T12:00:00Z", payload["claim"])
...
class TestSealIssueCommentPrediction(unittest.TestCase):
    def test_seals_a_real_predict_entry_to_a_scratch_ledger(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            entry = issue_comment_cadence.seal_issue_comment_prediction(
                now=_NOW,
    ...
    def test_a_tampered_sealed_issue_comment_prediction_breaks_verify(self):
        with tempfile.TemporaryDirectory() as tmp:
            ledger_path = os.path.join(tmp, "ledger.jsonl")
            mod = _fresh_ledger_module(ledger_path)
            issue_comment_cadence.seal_issue_comment_prediction(
                now=_NOW,
                ts=_NOW.isoformat(timespec="seconds"),
                current_count=0,
                snapshots=[],
