...
class TestListedCountAtOrBefore(unittest.TestCase):
    ...
    def test_raises_tampered_error_on_a_malformed_line_instead_of_crashing(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        with self.assertRaises(listed_cadence.ListedCadenceTamperedError):
            listed_cadence.listed_count_at_or_before(snaps, when)
...
class TestListedCountAtOrAfter(unittest.TestCase):
    ...
    def test_raises_tampered_error_on_a_malformed_line_instead_of_crashing(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        with self.assertRaises(listed_cadence.ListedCadenceTamperedError):
            listed_cadence.listed_count_at_or_after(snaps, when)
...
class TestBuildPrediction(unittest.TestCase):
    ...
    def test_non_utc_aware_now_still_targets_the_true_utc_instant(self):
        ...
        payload = listed_cadence.build_prediction(non_utc_now, [], current_count=2, horizon_hours=168)
        self.assertIn("2026-07-27T17:00:00Z", payload["claim"])

    def test_utc_now_unaffected_by_the_normalization(self):
        payload = listed_cadence.build_prediction(_NOW, [], current_count=2, horizon_hours=168)
        self.assertIn("2026-07-27T12:00:00Z", payload["claim"])
    ...
    def test_rejects_zero_horizon_hours(self):
        ...
        # be sealed.
        with self.assertRaises(listed_cadence.ListedCadenceError):
            listed_cadence.build_prediction(_NOW, [], current_count=1, horizon_hours=0)
    ...
    def test_rejects_negative_horizon_hours(self):
        ...
        with self.assertRaises(listed_cadence.ListedCadenceError):
            listed_cadence.build_prediction(_NOW, [], current_count=1, horizon_hours=-24)
