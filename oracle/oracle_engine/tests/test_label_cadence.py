"""Task 52. The Oracle Desk's sixteenth real cadence: one checkable claim
about the town's own public GitHub repository label count, sourced with no
Arcade scope at all (a repo's label list is public data), copylint-clean,
...
from oracle_engine import copylint, prediction, label_cadence  # noqa: E402
...
class TestFetchLabelCount(unittest.TestCase):
    def test_reads_the_list_length_off_a_single_page(self):
        def fake_get(url):
            self.assertEqual(
                url,
                "https://api.github.com/repos/thierrypdamiba/orita/labels?per_page=100&page=1",
            )
            return [{"name": "bug"}]

        self.assertEqual(label_cadence.fetch_label_count(http_get=fake_get), 1)
    ...
    def test_pages_until_a_short_page_ends_it(self):
        ...
        def fake_get(url):
            ...
            if page == 1:
                return [{"name": f"label-{i}"} for i in range(100)]
            return [{"name": "label-100"}, {"name": "label-101"}]

        self.assertEqual(label_cadence.fetch_label_count(http_get=fake_get), 102)
        self.assertEqual(len(calls), 2)

    def test_rejects_a_malformed_response(self):
        with self.assertRaises(label_cadence.LabelCadenceError):
            label_cadence.fetch_label_count(http_get=lambda url: {"nope": True})

    def test_empty_repo_is_zero(self):
        self.assertEqual(label_cadence.fetch_label_count(http_get=lambda url: []), 0)


class TestSnapshots(unittest.TestCase):
    def test_load_snapshots_of_a_missing_file_is_empty(self):
        self.assertEqual(label_cadence.load_snapshots("/does/not/exist.jsonl"), [])

    def test_record_then_load_round_trips(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = os.path.join(tmp, "snapshots.jsonl")
            label_cadence.record_snapshot(1, "2026-07-13T00:00:00+00:00", path=path)
            label_cadence.record_snapshot(2, "2026-07-14T00:00:00+00:00", path=path)
            snaps = label_cadence.load_snapshots(path)
...
class TestLabelCountAtOrBefore(unittest.TestCase):
    def test_returns_none_with_no_early_enough_snapshot(self):
        ...
        self.assertIsNone(label_cadence.label_count_at_or_before(snaps, when))
    ...
    def test_returns_the_latest_at_or_before(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        self.assertEqual(label_cadence.label_count_at_or_before(snaps, when), 1)
    ...
    def test_raises_tampered_error_on_a_malformed_line_instead_of_crashing(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        with self.assertRaises(label_cadence.LabelCadenceTamperedError):
            label_cadence.label_count_at_or_before(snaps, when)
...
class TestLabelCountAtOrAfter(unittest.TestCase):
    def test_returns_none_with_no_late_enough_snapshot(self):
        snaps = [{"ts": "2026-07-11T00:00:00+00:00", "count": 1}]
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        self.assertIsNone(label_cadence.label_count_at_or_after(snaps, when))
    ...
    def test_returns_the_earliest_at_or_after(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        self.assertEqual(label_cadence.label_count_at_or_after(snaps, when), 2)
    ...
    def test_raises_tampered_error_on_a_malformed_line_instead_of_crashing(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        with self.assertRaises(label_cadence.LabelCadenceTamperedError):
            label_cadence.label_count_at_or_after(snaps, when)
...
class TestBuildPrediction(unittest.TestCase):
    def test_claim_names_the_threshold_and_target(self):
        payload = label_cadence.build_prediction(_NOW, [], current_count=23, horizon_hours=336)
        self.assertIn("24", payload["claim"])
        self.assertIn("2026-08-03T12:00:00Z", payload["claim"])
        self.assertEqual(payload["confidence"], label_cadence.DEFAULT_CONFIDENCE)
    ...
    def test_a_baseline_snapshot_names_the_real_delta(self):
        ...
        snaps = [{"ts": baseline_when.isoformat(), "count": 23}]
        payload = label_cadence.build_prediction(_NOW, snaps, current_count=23, horizon_hours=336)
        self.assertIn("+0", payload["claim"])

    def test_claim_clears_copylint(self):
        payload = label_cadence.build_prediction(_NOW, [], current_count=23)
        result = copylint.enforce_copy(payload["claim"], payload["confidence"])
        self.assertTrue(result.ok)

    def test_rejects_naive_datetime(self):
        with self.assertRaises(label_cadence.LabelCadenceError):
            label_cadence.build_prediction(
                datetime.datetime(2026, 7, 20, 12, 0), [], current_count=23
            )
    ...
    def test_non_utc_aware_now_still_targets_the_true_utc_instant(self):
        ...
        payload = label_cadence.build_prediction(non_utc_now, [], current_count=23, horizon_hours=336)
    ...
    def test_rejects_a_negative_count(self):
        with self.assertRaises(label_cadence.LabelCadenceError):
            label_cadence.build_prediction(_NOW, [], current_count=-1)
    ...
    def test_rejects_zero_horizon_hours(self):
        ...
        # be sealed.
        with self.assertRaises(label_cadence.LabelCadenceError):
            label_cadence.build_prediction(_NOW, [], current_count=1, horizon_hours=0)
    ...
    def test_rejects_negative_horizon_hours(self):
        ...
        with self.assertRaises(label_cadence.LabelCadenceError):
            label_cadence.build_prediction(_NOW, [], current_count=1, horizon_hours=-24)


class TestSealLabelPrediction(unittest.TestCase):
    def test_seals_a_real_predict_entry_to_a_scratch_ledger(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            entry = label_cadence.seal_label_prediction(
                now=_NOW,
    ...
    def test_a_tampered_sealed_label_prediction_breaks_verify(self):
        with tempfile.TemporaryDirectory() as tmp:
            ledger_path = os.path.join(tmp, "ledger.jsonl")
            mod = _fresh_ledger_module(ledger_path)
            label_cadence.seal_label_prediction(
                now=_NOW,
                ts=_NOW.isoformat(timespec="seconds"),
                current_count=23,
                snapshots=[],
