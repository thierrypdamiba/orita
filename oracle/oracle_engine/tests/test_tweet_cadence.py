...
class TestSnapshots(unittest.TestCase):
    ...
    def test_load_snapshots_marks_a_malformed_line_instead_of_raising(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with open(path, "w", encoding="utf-8") as f:
                f.write('{"ts": "2026-07-13T00:00:00+00:00", "count": 21}\n')
                f.write("not valid json at all\n")
            snaps = tweet_cadence.load_snapshots(path)
            self.assertEqual(len(snaps), 2)
            self.assertEqual(snaps[0]["count"], 21)
    ...
    def test_a_valid_json_non_dict_snapshot_line_refuses_not_crashes(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaises(tweet_cadence.TweetCadenceTamperedError):
                tweet_cadence.tweet_count_at_or_before(snaps, when)
...
class TestTweetCountAtOrAfter(unittest.TestCase):
    ...
    def test_raises_tampered_error_on_a_malformed_line_instead_of_crashing(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        with self.assertRaises(tweet_cadence.TweetCadenceTamperedError):
            tweet_cadence.tweet_count_at_or_after(snaps, when)
