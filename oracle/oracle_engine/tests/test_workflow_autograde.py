"""Task 60's grading half: scoring a workflow-cadence prediction the moment
its target passes, off the real recorded snapshot log, exactly once, never
early, never on a guess. Ogun grades his own staked call under the same
discipline he has run on eighteen other gods' calls.
"""
...
from oracle_engine import grading, prediction, workflow_autograde, workflow_cadence  # noqa: E402
...
class AutogradeTestBase(unittest.TestCase):
    def setUp(self):
        self.ledger_path = os.path.join(_TESTS_DIR, "_scratch_workflow_autograde_ledger.jsonl")
        if os.path.exists(self.ledger_path):
            os.remove(self.ledger_path)
        self.mod = _fresh_ledger_module(self.ledger_path)
        self.snapshot_path = os.path.join(_TESTS_DIR, "_scratch_workflow_snapshots.jsonl")
    ...
    def _seal_workflow_call(self, sealed_at: str, target: str, threshold: int = 5, confidence: float = 0.5):
        claim = (
            f"By {target}, thierrypdamiba/orita's public GitHub Actions workflow count will be "
            f"at least {threshold} (currently {threshold - 1}, no earlier snapshot yet to compare against)."
        )
        return prediction.seal_prediction(
            "ogun", claim, confidence, ts=sealed_at, ledger_module=self.mod
        )


class TestParseWorkflowClaim(unittest.TestCase):
    def test_parses_target_and_threshold(self):
        claim = (
            "By 2026-08-03T12:00:00Z, thierrypdamiba/orita's public GitHub Actions workflow "
            "count will be at least 5 (currently 4, net change over the past 336h: +1)."
        )
        target, threshold = workflow_autograde.parse_workflow_claim(claim)
        self.assertEqual(threshold, 5)
        self.assertEqual(
            target, datetime.datetime(2026, 8, 3, 12, 0, tzinfo=datetime.timezone.utc)
        )

    def test_rejects_a_non_workflow_claim(self):
        with self.assertRaises(workflow_autograde.WorkflowAutogradeError):
            workflow_autograde.parse_workflow_claim("a fork opens within 30 days")


class TestFindDueCalls(AutogradeTestBase):
    def test_a_call_whose_window_has_not_closed_is_not_due(self):
        self._seal_workflow_call(sealed_at="2026-07-20T12:00:00+00:00", target="2026-08-03T12:00:00Z")
        now = datetime.datetime(2026, 7, 21, tzinfo=datetime.timezone.utc)
        due = workflow_autograde.find_due_calls(self.mod._entries(), now)
    ...
    def test_an_already_terminally_graded_call_is_not_due_again(self):
        call = self._seal_workflow_call(sealed_at="2026-07-20T12:00:00+00:00", target="2026-08-03T12:00:00Z")
        grading.seal_grade(
    ...
    def test_a_schema_mismatched_prior_grade_is_ignored_not_raised(self):
        ...
        self.mod.append("ogun", grading.GRADE_ACT, tampered_detail, "2026-08-03T13:00:00+00:00")
        now = datetime.datetime(2026, 8, 4, tzinfo=datetime.timezone.utc)
        due = workflow_autograde.find_due_calls(self.mod._entries(), now)
        self.assertEqual(len(due), 1)
...
class TestScoreCall(AutogradeTestBase):
    def test_scores_correct_when_the_recorded_snapshot_meets_threshold(self):
        call = self._seal_workflow_call(
            sealed_at="2026-07-20T12:00:00+00:00", target="2026-08-03T12:00:00Z", threshold=5
        )
        workflow_cadence.record_snapshot(5, "2026-08-03T13:00:00+00:00", path=self.snapshot_path)
        snapshots = workflow_cadence.load_snapshots(self.snapshot_path)
        self.assertEqual(workflow_autograde.score_call(call, snapshots), "correct")

    def test_scores_incorrect_when_the_recorded_snapshot_misses_threshold(self):
        call = self._seal_workflow_call(
            sealed_at="2026-07-20T12:00:00+00:00", target="2026-08-03T12:00:00Z", threshold=5
        )
        workflow_cadence.record_snapshot(4, "2026-08-03T13:00:00+00:00", path=self.snapshot_path)
        snapshots = workflow_cadence.load_snapshots(self.snapshot_path)
        self.assertEqual(workflow_autograde.score_call(call, snapshots), "incorrect")
    ...
    def test_raises_when_no_snapshot_exists_at_or_after_the_target(self):
        ...
        with self.assertRaises(workflow_autograde.WorkflowAutogradeError):
            workflow_autograde.score_call(call, [])
...
class TestAutogradeDuePredictions(AutogradeTestBase):
    def test_a_quiet_run_seals_nothing(self):
        self._seal_workflow_call(sealed_at="2026-07-20T12:00:00+00:00", target="2026-08-03T12:00:00Z")
        now = datetime.datetime(2026, 7, 21, tzinfo=datetime.timezone.utc)
        sealed = workflow_autograde.autograde_due_predictions(
    ...
    def test_running_twice_does_not_double_grade(self):
        self._seal_workflow_call(
            sealed_at="2026-07-20T12:00:00+00:00", target="2026-08-03T12:00:00Z", threshold=5
        )
        workflow_cadence.record_snapshot(5, "2026-08-03T13:00:00+00:00", path=self.snapshot_path)
        now = datetime.datetime(2026, 8, 4, tzinfo=datetime.timezone.utc)
        first = workflow_autograde.autograde_due_predictions(
            now=now, ts=now.isoformat(), snapshot_path=self.snapshot_path, ledger_module=self.mod,
        )
        second = workflow_autograde.autograde_due_predictions(
            now=now, ts=now.isoformat(), snapshot_path=self.snapshot_path, ledger_module=self.mod,
        )
    ...
    def test_now_must_be_timezone_aware(self):
        with self.assertRaises(workflow_autograde.WorkflowAutogradeError):
            workflow_autograde.autograde_due_predictions(
