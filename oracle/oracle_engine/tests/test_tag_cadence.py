"""Task 51. The Oracle Desk's fifteenth real cadence: one checkable claim
about the town's own public GitHub tag count, sourced with no Arcade scope
at all (a repo's tag list is public data), copylint-clean, sealed to a
...
from oracle_engine import copylint, prediction, tag_cadence  # noqa: E402
...
class TestFetchTagCount(unittest.TestCase):
    def test_reads_the_list_length_off_a_single_page(self):
        def fake_get(url):
            self.assertEqual(
                url,
                "https://api.github.com/repos/thierrypdamiba/orita/tags?per_page=100&page=1",
            )
            return [{"name": "v0.1"}]

        self.assertEqual(tag_cadence.fetch_tag_count(http_get=fake_get), 1)
    ...
    def test_pages_until_a_short_page_ends_it(self):
        ...
        def fake_get(url):
            ...
            if page == 1:
                return [{"name": f"v0.{i}"} for i in range(100)]
            return [{"name": "v0.100"}, {"name": "v0.101"}]

        self.assertEqual(tag_cadence.fetch_tag_count(http_get=fake_get), 102)
        self.assertEqual(len(calls), 2)

    def test_rejects_a_malformed_response(self):
        with self.assertRaises(tag_cadence.TagCadenceError):
            tag_cadence.fetch_tag_count(http_get=lambda url: {"nope": True})

    def test_empty_repo_is_zero(self):
        self.assertEqual(tag_cadence.fetch_tag_count(http_get=lambda url: []), 0)


class TestSnapshots(unittest.TestCase):
    def test_load_snapshots_of_a_missing_file_is_empty(self):
        self.assertEqual(tag_cadence.load_snapshots("/does/not/exist.jsonl"), [])

    def test_record_then_load_round_trips(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = os.path.join(tmp, "snapshots.jsonl")
            tag_cadence.record_snapshot(1, "2026-07-13T00:00:00+00:00", path=path)
            tag_cadence.record_snapshot(2, "2026-07-14T00:00:00+00:00", path=path)
            snaps = tag_cadence.load_snapshots(path)
...
class TestTagCountAtOrBefore(unittest.TestCase):
    def test_returns_none_with_no_early_enough_snapshot(self):
        ...
        self.assertIsNone(tag_cadence.tag_count_at_or_before(snaps, when))
    ...
    def test_returns_the_latest_at_or_before(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        self.assertEqual(tag_cadence.tag_count_at_or_before(snaps, when), 1)
    ...
    def test_raises_tampered_error_on_a_malformed_line_instead_of_crashing(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        with self.assertRaises(tag_cadence.TagCadenceTamperedError):
            tag_cadence.tag_count_at_or_before(snaps, when)


class TestTagCountAtOrAfter(unittest.TestCase):
    def test_returns_none_with_no_late_enough_snapshot(self):
        snaps = [{"ts": "2026-07-11T00:00:00+00:00", "count": 1}]
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        self.assertIsNone(tag_cadence.tag_count_at_or_after(snaps, when))
    ...
    def test_raises_tampered_error_on_a_malformed_line_instead_of_crashing(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        with self.assertRaises(tag_cadence.TagCadenceTamperedError):
            tag_cadence.tag_count_at_or_after(snaps, when)
    ...
    def test_returns_the_earliest_at_or_after(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        self.assertEqual(tag_cadence.tag_count_at_or_after(snaps, when), 2)
...
class TestBuildPrediction(unittest.TestCase):
    def test_claim_names_the_threshold_and_target(self):
        ...
        self.assertEqual(payload["confidence"], tag_cadence.DEFAULT_CONFIDENCE)
    ...
    def test_rejects_zero_horizon_hours(self):
        ...
        # be sealed.
        with self.assertRaises(tag_cadence.TagCadenceError):
            tag_cadence.build_prediction(_NOW, [], current_count=1, horizon_hours=0)
...
class TestSealTagPrediction(unittest.TestCase):
    def test_seals_a_real_predict_entry_to_a_scratch_ledger(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            entry = tag_cadence.seal_tag_prediction(
                now=_NOW,
    ...
    def test_a_tampered_sealed_tag_prediction_breaks_verify(self):
        with tempfile.TemporaryDirectory() as tmp:
            ledger_path = os.path.join(tmp, "ledger.jsonl")
            mod = _fresh_ledger_module(ledger_path)
            tag_cadence.seal_tag_prediction(
                now=_NOW,
                ts=_NOW.isoformat(timespec="seconds"),
                current_count=1,
                snapshots=[],
