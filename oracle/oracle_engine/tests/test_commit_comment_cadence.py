...
class TestCommitCommentCountAtOrBefore(unittest.TestCase):
    ...
    def test_a_valid_json_non_dict_snapshot_line_refuses_not_crashes(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            snaps = commit_comment_cadence.load_snapshots(path)
            when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
            with self.assertRaises(commit_comment_cadence.CommitCommentCadenceTamperedError):
                commit_comment_cadence.commit_comment_count_at_or_before(snaps, when)
...
class TestCommitCommentCountAtOrAfter(unittest.TestCase):
    ...
    def test_raises_tampered_error_on_a_malformed_line_instead_of_crashing(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        with self.assertRaises(commit_comment_cadence.CommitCommentCadenceTamperedError):
            commit_comment_cadence.commit_comment_count_at_or_after(snaps, when)
...
class TestBuildPrediction(unittest.TestCase):
    ...
    def test_rejects_zero_horizon_hours(self):
        ...
        # be sealed.
        with self.assertRaises(commit_comment_cadence.CommitCommentCadenceError):
            commit_comment_cadence.build_prediction(_NOW, [], current_count=1, horizon_hours=0)
    ...
    def test_rejects_negative_horizon_hours(self):
        ...
        with self.assertRaises(commit_comment_cadence.CommitCommentCadenceError):
            commit_comment_cadence.build_prediction(_NOW, [], current_count=1, horizon_hours=-24)
    ...
    def test_non_utc_aware_now_still_targets_the_true_utc_instant(self):
        ...
        payload = commit_comment_cadence.build_prediction(
            now_eastern, [], current_count=0, horizon_hours=336
        )
        self.assertIn("2026-08-03T12:00:00Z", payload["claim"])
