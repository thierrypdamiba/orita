"""Task 40. The Oracle Desk's fourth real cadence: one checkable claim
about the town's own public GitHub open-issue count, sourced with no
Arcade scope at all (a repo's open-issue count is public data),
...
from oracle_engine import copylint, issue_cadence, prediction  # noqa: E402
...
class TestFetchOpenIssueCount(unittest.TestCase):
    def test_reads_open_issues_count_off_the_injected_getter(self):
        def fake_get(url):
            self.assertEqual(url, "https://api.github.com/repos/thierrypdamiba/orita")
            return {"open_issues_count": 4}

        self.assertEqual(issue_cadence.fetch_open_issue_count(http_get=fake_get), 4)

    def test_rejects_a_malformed_response(self):
        with self.assertRaises(issue_cadence.IssueCadenceError):
            issue_cadence.fetch_open_issue_count(http_get=lambda url: {"nope": True})


class TestSnapshots(unittest.TestCase):
    def test_load_snapshots_of_a_missing_file_is_empty(self):
        self.assertEqual(issue_cadence.load_snapshots("/does/not/exist.jsonl"), [])

    def test_record_then_load_round_trips(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = os.path.join(tmp, "snapshots.jsonl")
            issue_cadence.record_snapshot(4, "2026-07-13T00:00:00+00:00", path=path)
            issue_cadence.record_snapshot(5, "2026-07-14T00:00:00+00:00", path=path)
            snaps = issue_cadence.load_snapshots(path)
            self.assertEqual([s["count"] for s in snaps], [4, 5])

    def test_record_rejects_a_negative_count(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = os.path.join(tmp, "snapshots.jsonl")
            with self.assertRaises(issue_cadence.IssueCadenceError):
                issue_cadence.record_snapshot(-1, "2026-07-13T00:00:00+00:00", path=path)
...
class TestIssueCountAtOrBefore(unittest.TestCase):
    def test_returns_none_with_no_early_enough_snapshot(self):
        snaps = [{"ts": "2026-07-14T00:00:00+00:00", "count": 4}]
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        self.assertIsNone(issue_cadence.issue_count_at_or_before(snaps, when))

    def test_returns_the_latest_at_or_before(self):
        snaps = [
            {"ts": "2026-07-11T00:00:00+00:00", "count": 3},
            {"ts": "2026-07-12T00:00:00+00:00", "count": 4},
            {"ts": "2026-07-14T00:00:00+00:00", "count": 5},
        ]
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        self.assertEqual(issue_cadence.issue_count_at_or_before(snaps, when), 4)
    ...
    def test_raises_tampered_error_on_a_malformed_line_instead_of_crashing(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        with self.assertRaises(issue_cadence.IssueCadenceTamperedError):
            issue_cadence.issue_count_at_or_before(snaps, when)


class TestIssueCountAtOrAfter(unittest.TestCase):
    def test_returns_none_with_no_late_enough_snapshot(self):
        snaps = [{"ts": "2026-07-11T00:00:00+00:00", "count": 3}]
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        self.assertIsNone(issue_cadence.issue_count_at_or_after(snaps, when))

    def test_returns_the_earliest_at_or_after(self):
        snaps = [
            {"ts": "2026-07-11T00:00:00+00:00", "count": 3},
            {"ts": "2026-07-14T00:00:00+00:00", "count": 5},
            {"ts": "2026-07-16T00:00:00+00:00", "count": 6},
        ]
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        self.assertEqual(issue_cadence.issue_count_at_or_after(snaps, when), 5)
    ...
    def test_raises_tampered_error_on_a_malformed_line_instead_of_crashing(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        with self.assertRaises(issue_cadence.IssueCadenceTamperedError):
            issue_cadence.issue_count_at_or_after(snaps, when)
...
class TestBuildPrediction(unittest.TestCase):
    def test_claim_names_the_threshold_and_target(self):
        payload = issue_cadence.build_prediction(_NOW, [], current_count=4, horizon_hours=336)
        self.assertIn("5", payload["claim"])
        self.assertIn("2026-08-03T12:00:00Z", payload["claim"])
        self.assertEqual(payload["confidence"], issue_cadence.DEFAULT_CONFIDENCE)
    ...
    def test_a_baseline_snapshot_names_the_real_delta(self):
        ...
        snaps = [{"ts": baseline_when.isoformat(), "count": 3}]
        payload = issue_cadence.build_prediction(_NOW, snaps, current_count=4, horizon_hours=336)
        self.assertIn("+1", payload["claim"])

    def test_claim_clears_copylint(self):
        payload = issue_cadence.build_prediction(_NOW, [], current_count=4)
        result = copylint.enforce_copy(payload["claim"], payload["confidence"])
        self.assertTrue(result.ok)

    def test_rejects_naive_datetime(self):
        with self.assertRaises(issue_cadence.IssueCadenceError):
            issue_cadence.build_prediction(
                datetime.datetime(2026, 7, 20, 12, 0), [], current_count=4
            )

    def test_rejects_a_negative_count(self):
        with self.assertRaises(issue_cadence.IssueCadenceError):
            issue_cadence.build_prediction(_NOW, [], current_count=-1)
    ...
    def test_rejects_zero_horizon_hours(self):
        ...
        # be sealed.
        with self.assertRaises(issue_cadence.IssueCadenceError):
            issue_cadence.build_prediction(_NOW, [], current_count=1, horizon_hours=0)
    ...
    def test_rejects_negative_horizon_hours(self):
        ...
        with self.assertRaises(issue_cadence.IssueCadenceError):
            issue_cadence.build_prediction(_NOW, [], current_count=1, horizon_hours=-24)
    ...
    def test_non_utc_aware_now_still_targets_the_true_utc_instant(self):
        ...
        payload = issue_cadence.build_prediction(
            now_eastern, [], current_count=4, horizon_hours=336
        )
        self.assertIn("2026-08-03T12:00:00Z", payload["claim"])
...
class TestSealIssuePrediction(unittest.TestCase):
    def test_seals_a_real_predict_entry_to_a_scratch_ledger(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            entry = issue_cadence.seal_issue_prediction(
                now=_NOW,
    ...
    def test_a_tampered_sealed_issue_prediction_breaks_verify(self):
        with tempfile.TemporaryDirectory() as tmp:
            ledger_path = os.path.join(tmp, "ledger.jsonl")
            mod = _fresh_ledger_module(ledger_path)
            issue_cadence.seal_issue_prediction(
                now=_NOW,
                ts=_NOW.isoformat(timespec="seconds"),
                current_count=4,
                snapshots=[],
