...
class TestContributorCountAtOrBefore(unittest.TestCase):
    ...
    def test_raises_tampered_error_on_a_malformed_line_instead_of_crashing(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        with self.assertRaises(contributor_cadence.ContributorCadenceTamperedError):
            contributor_cadence.contributor_count_at_or_before(snaps, when)
...
class TestContributorCountAtOrAfter(unittest.TestCase):
    ...
    def test_raises_tampered_error_on_a_malformed_line_instead_of_crashing(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        with self.assertRaises(contributor_cadence.ContributorCadenceTamperedError):
            contributor_cadence.contributor_count_at_or_after(snaps, when)
...
class TestBuildPrediction(unittest.TestCase):
    ...
    def test_non_utc_aware_now_still_targets_the_true_utc_instant(self):
        ...
        payload = contributor_cadence.build_prediction(
    ...
    def test_utc_now_unaffected_by_the_normalization(self):
        ...
        payload = contributor_cadence.build_prediction(_NOW, [], current_count=0, horizon_hours=336)
        self.assertIn("2026-08-03T12:00:00Z", payload["claim"])
