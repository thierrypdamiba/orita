"""Task 45's grading half: scoring a media-cadence prediction the moment
...
from oracle_engine import grading, media_autograde, media_cadence, prediction  # noqa: E402
...
class AutogradeTestBase(unittest.TestCase):
    def setUp(self):
        self.ledger_path = os.path.join(_TESTS_DIR, "_scratch_media_autograde_ledger.jsonl")
        if os.path.exists(self.ledger_path):
            os.remove(self.ledger_path)
        self.mod = _fresh_ledger_module(self.ledger_path)
        self.snapshot_path = os.path.join(_TESTS_DIR, "_scratch_media_snapshots.jsonl")
    ...
    def _seal_media_call(self, sealed_at: str, target: str, threshold: int = 1, confidence: float = 0.55):
        claim = (
            f"By {target}, @oritatown's public X media count will be at "
...
class TestParseMediaClaim(unittest.TestCase):
    def test_parses_target_and_threshold(self):
        claim = (
            "By 2026-07-27T12:00:00Z, @oritatown's public X media count "
            "will be at least 1 (currently 0, net change over the past 168h: +1)."
        )
        target, threshold = media_autograde.parse_media_claim(claim)
        self.assertEqual(threshold, 1)
        self.assertEqual(
            target, datetime.datetime(2026, 7, 27, 12, 0, tzinfo=datetime.timezone.utc)
        )

    def test_rejects_a_non_media_claim(self):
        with self.assertRaises(media_autograde.MediaAutogradeError):
            media_autograde.parse_media_claim("a star opens within 30 days")


class TestFindDueCalls(AutogradeTestBase):
    def test_a_call_whose_window_has_not_closed_is_not_due(self):
        self._seal_media_call(sealed_at="2026-07-20T12:00:00+00:00", target="2026-07-27T12:00:00Z")
        now = datetime.datetime(2026, 7, 21, tzinfo=datetime.timezone.utc)
        due = media_autograde.find_due_calls(self.mod._entries(), now)
    ...
    def test_an_already_terminally_graded_call_is_not_due_again(self):
        call = self._seal_media_call(sealed_at="2026-07-20T12:00:00+00:00", target="2026-07-27T12:00:00Z")
        grading.seal_grade(
    ...
    def test_a_listed_cadence_shaped_prediction_is_skipped_not_raised(self):
        prediction.seal_prediction(
            "kwaku-ananse",
            "By 2026-07-27T12:00:00Z, @oritatown's public X listed count "
            "will be at least 1 (currently 0, no earlier snapshot yet to compare against).",
            0.55,
...
class TestScoreCall(AutogradeTestBase):
    def test_scores_correct_when_the_recorded_snapshot_meets_threshold(self):
        call = self._seal_media_call(
            sealed_at="2026-07-20T12:00:00+00:00", target="2026-07-27T12:00:00Z", threshold=1
        )
        media_cadence.record_snapshot(1, "2026-07-27T13:00:00+00:00", path=self.snapshot_path)
        snapshots = media_cadence.load_snapshots(self.snapshot_path)
        self.assertEqual(media_autograde.score_call(call, snapshots), "correct")

    def test_scores_incorrect_when_the_recorded_snapshot_misses_threshold(self):
        call = self._seal_media_call(
            sealed_at="2026-07-20T12:00:00+00:00", target="2026-07-27T12:00:00Z", threshold=1
        )
        media_cadence.record_snapshot(0, "2026-07-27T13:00:00+00:00", path=self.snapshot_path)
        snapshots = media_cadence.load_snapshots(self.snapshot_path)
        self.assertEqual(media_autograde.score_call(call, snapshots), "incorrect")
    ...
    def test_raises_when_no_snapshot_exists_at_or_after_the_target(self):
        ...
        with self.assertRaises(media_autograde.MediaAutogradeError):
            media_autograde.score_call(call, [])
...
class TestAutogradeDuePredictions(AutogradeTestBase):
    def test_a_quiet_run_seals_nothing(self):
        self._seal_media_call(sealed_at="2026-07-20T12:00:00+00:00", target="2026-07-27T12:00:00Z")
        now = datetime.datetime(2026, 7, 21, tzinfo=datetime.timezone.utc)
        sealed = media_autograde.autograde_due_predictions(
    ...
    def test_running_twice_does_not_double_grade(self):
        self._seal_media_call(
            sealed_at="2026-07-20T12:00:00+00:00", target="2026-07-27T12:00:00Z", threshold=1
        )
        media_cadence.record_snapshot(1, "2026-07-27T13:00:00+00:00", path=self.snapshot_path)
        now = datetime.datetime(2026, 7, 28, tzinfo=datetime.timezone.utc)
        first = media_autograde.autograde_due_predictions(
            now=now, ts=now.isoformat(), snapshot_path=self.snapshot_path, ledger_module=self.mod,
        )
        second = media_autograde.autograde_due_predictions(
            now=now, ts=now.isoformat(), snapshot_path=self.snapshot_path, ledger_module=self.mod,
        )
    ...
    def test_now_must_be_timezone_aware(self):
        with self.assertRaises(media_autograde.MediaAutogradeError):
            media_autograde.autograde_due_predictions(
