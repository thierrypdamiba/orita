"""Task 76. The Oracle Desk's twenty-third real cadence: one checkable
claim about the town's own public GitHub pull-request review comment
count, sourced with no Arcade scope at all (a repo's PR review comments
are public data), copylint-clean, sealed to a real (scratch, in these
...
from oracle_engine import comment_cadence, copylint, prediction  # noqa: E402
...
class TestFetchCommentCount(unittest.TestCase):
    def test_reads_the_list_length_off_a_single_page(self):
        def fake_get(url):
            self.assertEqual(
                url,
                "https://api.github.com/repos/thierrypdamiba/orita/pulls/comments?per_page=100&page=1",
            )
            return [{"id": 1}]

        self.assertEqual(comment_cadence.fetch_comment_count(http_get=fake_get), 1)
    ...
    def test_pages_until_a_short_page_ends_it(self):
        ...
        self.assertEqual(comment_cadence.fetch_comment_count(http_get=fake_get), 102)
        self.assertEqual(len(calls), 2)

    def test_rejects_a_malformed_response(self):
        with self.assertRaises(comment_cadence.CommentCadenceError):
            comment_cadence.fetch_comment_count(http_get=lambda url: {"nope": True})

    def test_no_review_comments_yet_is_zero(self):
        self.assertEqual(comment_cadence.fetch_comment_count(http_get=lambda url: []), 0)


class TestSnapshots(unittest.TestCase):
    def test_load_snapshots_of_a_missing_file_is_empty(self):
        self.assertEqual(comment_cadence.load_snapshots("/does/not/exist.jsonl"), [])

    def test_record_then_load_round_trips(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = os.path.join(tmp, "snapshots.jsonl")
            comment_cadence.record_snapshot(0, "2026-07-13T00:00:00+00:00", path=path)
            comment_cadence.record_snapshot(1, "2026-07-14T00:00:00+00:00", path=path)
            snaps = comment_cadence.load_snapshots(path)
    ...
    def test_record_rejects_a_negative_count(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = os.path.join(tmp, "snapshots.jsonl")
            with self.assertRaises(comment_cadence.CommentCadenceError):
                comment_cadence.record_snapshot(-1, "2026-07-13T00:00:00+00:00", path=path)
...
class TestCommentCountAtOrBefore(unittest.TestCase):
    def test_returns_none_with_no_early_enough_snapshot(self):
        ...
        self.assertIsNone(comment_cadence.comment_count_at_or_before(snaps, when))
    ...
    def test_returns_the_latest_at_or_before(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        self.assertEqual(comment_cadence.comment_count_at_or_before(snaps, when), 0)
    ...
    def test_raises_tampered_error_on_a_malformed_line_instead_of_crashing(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        with self.assertRaises(comment_cadence.CommentCadenceTamperedError):
            comment_cadence.comment_count_at_or_before(snaps, when)
...
class TestCommentCountAtOrAfter(unittest.TestCase):
    def test_returns_none_with_no_late_enough_snapshot(self):
        snaps = [{"ts": "2026-07-11T00:00:00+00:00", "count": 0}]
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        self.assertIsNone(comment_cadence.comment_count_at_or_after(snaps, when))
    ...
    def test_raises_tampered_error_on_a_malformed_line_instead_of_crashing(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        with self.assertRaises(comment_cadence.CommentCadenceTamperedError):
            comment_cadence.comment_count_at_or_after(snaps, when)
    ...
    def test_returns_the_earliest_at_or_after(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        self.assertEqual(comment_cadence.comment_count_at_or_after(snaps, when), 1)


class TestBuildPrediction(unittest.TestCase):
    def test_claim_names_the_threshold_and_target(self):
        payload = comment_cadence.build_prediction(_NOW, [], current_count=0, horizon_hours=336)
        self.assertIn("1", payload["claim"])
        self.assertIn("2026-08-03T12:00:00Z", payload["claim"])
        self.assertEqual(payload["confidence"], comment_cadence.DEFAULT_CONFIDENCE)
    ...
    def test_a_baseline_snapshot_names_the_real_delta(self):
        ...
        payload = comment_cadence.build_prediction(_NOW, snaps, current_count=0, horizon_hours=336)
        self.assertIn("+0", payload["claim"])

    def test_claim_clears_copylint(self):
        payload = comment_cadence.build_prediction(_NOW, [], current_count=0)
        result = copylint.enforce_copy(payload["claim"], payload["confidence"])
        self.assertTrue(result.ok)

    def test_rejects_naive_datetime(self):
        with self.assertRaises(comment_cadence.CommentCadenceError):
            comment_cadence.build_prediction(
                datetime.datetime(2026, 7, 20, 12, 0), [], current_count=0
            )

    def test_rejects_a_negative_count(self):
        with self.assertRaises(comment_cadence.CommentCadenceError):
            comment_cadence.build_prediction(_NOW, [], current_count=-1)
    ...
    def test_rejects_zero_horizon_hours(self):
        ...
        # be sealed.
        with self.assertRaises(comment_cadence.CommentCadenceError):
            comment_cadence.build_prediction(_NOW, [], current_count=1, horizon_hours=0)
    ...
    def test_rejects_negative_horizon_hours(self):
        ...
        with self.assertRaises(comment_cadence.CommentCadenceError):
            comment_cadence.build_prediction(_NOW, [], current_count=1, horizon_hours=-24)
    ...
    def test_non_utc_aware_now_still_targets_the_true_utc_instant(self):
        ...
        payload = comment_cadence.build_prediction(now_eastern, [], current_count=4, horizon_hours=168)
    ...
    def test_utc_now_unaffected_by_the_normalization(self):
        ...
        payload = comment_cadence.build_prediction(_NOW, [], current_count=4, horizon_hours=168)
        self.assertIn("2026-07-27T12:00:00Z", payload["claim"])


class TestSealCommentPrediction(unittest.TestCase):
    def test_seals_a_real_predict_entry_to_a_scratch_ledger(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            entry = comment_cadence.seal_comment_prediction(
                now=_NOW,
    ...
    def test_a_tampered_sealed_comment_prediction_breaks_verify(self):
        with tempfile.TemporaryDirectory() as tmp:
            ledger_path = os.path.join(tmp, "ledger.jsonl")
            mod = _fresh_ledger_module(ledger_path)
            comment_cadence.seal_comment_prediction(
                now=_NOW,
                ts=_NOW.isoformat(timespec="seconds"),
                current_count=0,
                snapshots=[],
