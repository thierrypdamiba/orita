"""Task 41. The Oracle Desk's fifth real cadence: one checkable claim
about the town's own public GitHub release count, sourced with no Arcade
scope at all (a repo's release list is public data), copylint-clean,
...
from oracle_engine import copylint, prediction, release_cadence  # noqa: E402
...
class TestFetchReleaseCount(unittest.TestCase):
    def test_reads_the_list_length_off_a_single_page(self):
        def fake_get(url):
            self.assertEqual(
                url,
                "https://api.github.com/repos/thierrypdamiba/orita/releases?per_page=100&page=1",
            )
            return [{"tag_name": "episode-001"}]

        self.assertEqual(release_cadence.fetch_release_count(http_get=fake_get), 1)
    ...
    def test_pages_until_a_short_page_ends_it(self):
        ...
        def fake_get(url):
            ...
            if page == 1:
                return [{"tag_name": f"r{i}"} for i in range(100)]
            return [{"tag_name": "r100"}, {"tag_name": "r101"}]

        self.assertEqual(release_cadence.fetch_release_count(http_get=fake_get), 102)
        self.assertEqual(len(calls), 2)

    def test_rejects_a_malformed_response(self):
        with self.assertRaises(release_cadence.ReleaseCadenceError):
            release_cadence.fetch_release_count(http_get=lambda url: {"nope": True})

    def test_empty_repo_is_zero(self):
        self.assertEqual(release_cadence.fetch_release_count(http_get=lambda url: []), 0)


class TestSnapshots(unittest.TestCase):
    def test_load_snapshots_of_a_missing_file_is_empty(self):
        self.assertEqual(release_cadence.load_snapshots("/does/not/exist.jsonl"), [])

    def test_record_then_load_round_trips(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = os.path.join(tmp, "snapshots.jsonl")
            release_cadence.record_snapshot(1, "2026-07-13T00:00:00+00:00", path=path)
            release_cadence.record_snapshot(2, "2026-07-14T00:00:00+00:00", path=path)
            snaps = release_cadence.load_snapshots(path)
    ...
    def test_never_rewrites_a_prior_line(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = os.path.join(tmp, "snapshots.jsonl")
            release_cadence.record_snapshot(0, "2026-07-13T00:00:00+00:00", path=path)
            with open(path) as f:
                first_write = f.read()
            release_cadence.record_snapshot(1, "2026-07-14T00:00:00+00:00", path=path)
            with open(path) as f:
                second_write = f.read()
    ...
    def test_a_valid_json_non_dict_snapshot_line_refuses_not_crashes(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaises(release_cadence.ReleaseCadenceTamperedError):
                release_cadence.release_count_at_or_before(snaps, when)


class TestReleaseCountAtOrBefore(unittest.TestCase):
    def test_returns_none_with_no_early_enough_snapshot(self):
        snaps = [{"ts": "2026-07-14T00:00:00+00:00", "count": 2}]
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        self.assertIsNone(release_cadence.release_count_at_or_before(snaps, when))

    def test_returns_the_latest_at_or_before(self):
        snaps = [
            {"ts": "2026-07-11T00:00:00+00:00", "count": 0},
            {"ts": "2026-07-12T00:00:00+00:00", "count": 1},
            {"ts": "2026-07-14T00:00:00+00:00", "count": 3},
        ]
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        self.assertEqual(release_cadence.release_count_at_or_before(snaps, when), 1)
...
class TestReleaseCountAtOrAfter(unittest.TestCase):
    def test_returns_none_with_no_late_enough_snapshot(self):
        snaps = [{"ts": "2026-07-11T00:00:00+00:00", "count": 0}]
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        self.assertIsNone(release_cadence.release_count_at_or_after(snaps, when))

    def test_returns_the_earliest_at_or_after(self):
        snaps = [
            {"ts": "2026-07-11T00:00:00+00:00", "count": 0},
            {"ts": "2026-07-14T00:00:00+00:00", "count": 3},
            {"ts": "2026-07-16T00:00:00+00:00", "count": 4},
        ]
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        self.assertEqual(release_cadence.release_count_at_or_after(snaps, when), 3)
    ...
    def test_raises_tampered_error_on_a_malformed_line_instead_of_crashing(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        with self.assertRaises(release_cadence.ReleaseCadenceTamperedError):
            release_cadence.release_count_at_or_after(snaps, when)
...
class TestBuildPrediction(unittest.TestCase):
    def test_claim_names_the_threshold_and_target(self):
        payload = release_cadence.build_prediction(_NOW, [], current_count=1, horizon_hours=336)
        self.assertIn("2", payload["claim"])
        self.assertIn("2026-08-03T12:00:00Z", payload["claim"])
        self.assertEqual(payload["confidence"], release_cadence.DEFAULT_CONFIDENCE)
    ...
    def test_a_baseline_snapshot_names_the_real_delta(self):
        ...
        payload = release_cadence.build_prediction(_NOW, snaps, current_count=1, horizon_hours=336)
        self.assertIn("+1", payload["claim"])

    def test_claim_clears_copylint(self):
        payload = release_cadence.build_prediction(_NOW, [], current_count=1)
        result = copylint.enforce_copy(payload["claim"], payload["confidence"])
        self.assertTrue(result.ok)

    def test_rejects_naive_datetime(self):
        with self.assertRaises(release_cadence.ReleaseCadenceError):
            release_cadence.build_prediction(
    ...
    def test_non_utc_aware_now_still_targets_the_true_utc_instant(self):
        ...
        payload = release_cadence.build_prediction(now, [], current_count=1, horizon_hours=336)
    ...
    def test_rejects_a_negative_count(self):
        with self.assertRaises(release_cadence.ReleaseCadenceError):
            release_cadence.build_prediction(_NOW, [], current_count=-1)
    ...
    def test_rejects_zero_horizon_hours(self):
        ...
        # be sealed.
        with self.assertRaises(release_cadence.ReleaseCadenceError):
            release_cadence.build_prediction(_NOW, [], current_count=1, horizon_hours=0)
    ...
    def test_rejects_negative_horizon_hours(self):
        ...
        with self.assertRaises(release_cadence.ReleaseCadenceError):
            release_cadence.build_prediction(_NOW, [], current_count=1, horizon_hours=-24)


class TestSealReleasePrediction(unittest.TestCase):
    def test_seals_a_real_predict_entry_to_a_scratch_ledger(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            entry = release_cadence.seal_release_prediction(
                now=_NOW,
    ...
    def test_a_tampered_sealed_release_prediction_breaks_verify(self):
        with tempfile.TemporaryDirectory() as tmp:
            ledger_path = os.path.join(tmp, "ledger.jsonl")
            mod = _fresh_ledger_module(ledger_path)
            release_cadence.seal_release_prediction(
                now=_NOW,
                ts=_NOW.isoformat(timespec="seconds"),
                current_count=1,
                snapshots=[],
