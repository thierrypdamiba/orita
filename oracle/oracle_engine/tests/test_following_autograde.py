"""Task 46's grading half: scoring a following-cadence prediction the
moment its target passes, off the real recorded snapshot log, exactly
once, never early, never on a guess. Unlike every prior X-metrics
cadence, this claim names an UPPER bound, so `score_call` grades the
inverse comparison on purpose.
"""
...
from oracle_engine import following_autograde, following_cadence, grading, prediction  # noqa: E402
...
class AutogradeTestBase(unittest.TestCase):
    def setUp(self):
        self.ledger_path = os.path.join(_TESTS_DIR, "_scratch_following_autograde_ledger.jsonl")
        if os.path.exists(self.ledger_path):
            os.remove(self.ledger_path)
        self.mod = _fresh_ledger_module(self.ledger_path)
        self.snapshot_path = os.path.join(_TESTS_DIR, "_scratch_following_snapshots.jsonl")
    ...
    def _seal_following_call(self, sealed_at: str, target: str, threshold: int = 0, confidence: float = 0.6):
        claim = (
            f"By {target}, @oritatown's public X following count will be no "
            f"more than {threshold} (currently {threshold}, no earlier snapshot yet to compare against)."
        )
        return prediction.seal_prediction(
            "retrya", claim, confidence, ts=sealed_at, ledger_module=self.mod
        )


class TestParseFollowingClaim(unittest.TestCase):
    def test_parses_target_and_threshold(self):
        claim = (
            "By 2026-07-27T12:00:00Z, @oritatown's public X following count "
            "will be no more than 0 (currently 0, net change over the past 168h: +0)."
        )
        target, threshold = following_autograde.parse_following_claim(claim)
        self.assertEqual(threshold, 0)
        self.assertEqual(
            target, datetime.datetime(2026, 7, 27, 12, 0, tzinfo=datetime.timezone.utc)
        )

    def test_rejects_a_non_following_claim(self):
        with self.assertRaises(following_autograde.FollowingAutogradeError):
            following_autograde.parse_following_claim("a star opens within 30 days")


class TestFindDueCalls(AutogradeTestBase):
    def test_a_call_whose_window_has_not_closed_is_not_due(self):
        self._seal_following_call(sealed_at="2026-07-20T12:00:00+00:00", target="2026-07-27T12:00:00Z")
    ...
    def test_an_already_terminally_graded_call_is_not_due_again(self):
        call = self._seal_following_call(sealed_at="2026-07-20T12:00:00+00:00", target="2026-07-27T12:00:00Z")
        grading.seal_grade(
    ...
    def test_a_follower_cadence_shaped_prediction_is_skipped_not_raised(self):
        prediction.seal_prediction(
            "kwaku-ananse",
            "By 2026-07-27T12:00:00Z, @oritatown's public X follower count "
...
class TestScoreCall(AutogradeTestBase):
    def test_scores_correct_when_the_recorded_snapshot_stays_at_or_under_threshold(self):
        call = self._seal_following_call(
            sealed_at="2026-07-20T12:00:00+00:00", target="2026-07-27T12:00:00Z", threshold=0
        )
        following_cadence.record_snapshot(0, "2026-07-27T13:00:00+00:00", path=self.snapshot_path)
        snapshots = following_cadence.load_snapshots(self.snapshot_path)
        self.assertEqual(following_autograde.score_call(call, snapshots), "correct")

    def test_scores_incorrect_when_the_recorded_snapshot_exceeds_threshold(self):
        call = self._seal_following_call(
            sealed_at="2026-07-20T12:00:00+00:00", target="2026-07-27T12:00:00Z", threshold=0
        )
        following_cadence.record_snapshot(1, "2026-07-27T13:00:00+00:00", path=self.snapshot_path)
        snapshots = following_cadence.load_snapshots(self.snapshot_path)
        self.assertEqual(following_autograde.score_call(call, snapshots), "incorrect")
    ...
    def test_raises_when_no_snapshot_exists_at_or_after_the_target(self):
        ...
        with self.assertRaises(following_autograde.FollowingAutogradeError):
            following_autograde.score_call(call, [])
...
class TestAutogradeDuePredictions(AutogradeTestBase):
    def test_a_quiet_run_seals_nothing(self):
        self._seal_following_call(sealed_at="2026-07-20T12:00:00+00:00", target="2026-07-27T12:00:00Z")
        now = datetime.datetime(2026, 7, 21, tzinfo=datetime.timezone.utc)
        sealed = following_autograde.autograde_due_predictions(
    ...
    def test_running_twice_does_not_double_grade(self):
        self._seal_following_call(
            sealed_at="2026-07-20T12:00:00+00:00", target="2026-07-27T12:00:00Z", threshold=0
        )
        following_cadence.record_snapshot(0, "2026-07-27T13:00:00+00:00", path=self.snapshot_path)
        now = datetime.datetime(2026, 7, 28, tzinfo=datetime.timezone.utc)
        first = following_autograde.autograde_due_predictions(
            now=now, ts=now.isoformat(), snapshot_path=self.snapshot_path, ledger_module=self.mod,
        )
        second = following_autograde.autograde_due_predictions(
            now=now, ts=now.isoformat(), snapshot_path=self.snapshot_path, ledger_module=self.mod,
        )
    ...
    def test_now_must_be_timezone_aware(self):
        with self.assertRaises(following_autograde.FollowingAutogradeError):
            following_autograde.autograde_due_predictions(
