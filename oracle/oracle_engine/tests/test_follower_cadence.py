...
class TestFollowerCountAtOrBefore(unittest.TestCase):
    ...
    def test_raises_tampered_error_on_a_malformed_line_instead_of_crashing(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        with self.assertRaises(follower_cadence.FollowerCadenceTamperedError):
            follower_cadence.follower_count_at_or_before(snaps, when)
...
class TestFollowerCountAtOrAfter(unittest.TestCase):
    ...
    def test_raises_tampered_error_on_a_malformed_line_instead_of_crashing(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        with self.assertRaises(follower_cadence.FollowerCadenceTamperedError):
            follower_cadence.follower_count_at_or_after(snaps, when)
