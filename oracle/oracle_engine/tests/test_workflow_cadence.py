"""Task 60. The Oracle Desk's nineteenth real cadence: one checkable claim
about the town's own public GitHub Actions workflow count, sourced with no
Arcade scope at all (a repo's workflow count is public data), copylint-clean,
sealed to a real (scratch, in these tests) ledger before its outcome is
knowable. Ogun's first staked call, not just his nineteenth grade.
"""
...
from oracle_engine import copylint, prediction, workflow_cadence  # noqa: E402
...
class TestFetchWorkflowCount(unittest.TestCase):
    def test_reads_total_count_off_the_injected_getter(self):
        def fake_get(url):
            self.assertEqual(
                url, "https://api.github.com/repos/thierrypdamiba/orita/actions/workflows"
            )
            return {"total_count": 4, "workflows": []}

        self.assertEqual(workflow_cadence.fetch_workflow_count(http_get=fake_get), 4)

    def test_rejects_a_malformed_response(self):
        with self.assertRaises(workflow_cadence.WorkflowCadenceError):
            workflow_cadence.fetch_workflow_count(http_get=lambda url: {"nope": True})


class TestSnapshots(unittest.TestCase):
    def test_load_snapshots_of_a_missing_file_is_empty(self):
        self.assertEqual(workflow_cadence.load_snapshots("/does/not/exist.jsonl"), [])

    def test_record_then_load_round_trips(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = os.path.join(tmp, "snapshots.jsonl")
            workflow_cadence.record_snapshot(3, "2026-07-13T00:00:00+00:00", path=path)
            workflow_cadence.record_snapshot(4, "2026-07-14T00:00:00+00:00", path=path)
            snaps = workflow_cadence.load_snapshots(path)
            self.assertEqual([s["count"] for s in snaps], [3, 4])

    def test_record_rejects_a_negative_count(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = os.path.join(tmp, "snapshots.jsonl")
            with self.assertRaises(workflow_cadence.WorkflowCadenceError):
                workflow_cadence.record_snapshot(-1, "2026-07-13T00:00:00+00:00", path=path)
    ...
    def test_never_rewrites_a_prior_line(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with open(path) as f:
                first_write = f.read()
            workflow_cadence.record_snapshot(2, "2026-07-14T00:00:00+00:00", path=path)
            with open(path) as f:
                second_write = f.read()
...
class TestWorkflowCountAtOrBefore(unittest.TestCase):
    def test_returns_none_with_no_early_enough_snapshot(self):
        ...
        self.assertIsNone(workflow_cadence.workflow_count_at_or_before(snaps, when))

    def test_returns_the_latest_at_or_before(self):
        snaps = [
            {"ts": "2026-07-11T00:00:00+00:00", "count": 1},
            {"ts": "2026-07-12T00:00:00+00:00", "count": 3},
            {"ts": "2026-07-14T00:00:00+00:00", "count": 4},
        ]
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        self.assertEqual(workflow_cadence.workflow_count_at_or_before(snaps, when), 3)
...
class TestWorkflowCountAtOrAfter(unittest.TestCase):
    def test_returns_none_with_no_late_enough_snapshot(self):
        snaps = [{"ts": "2026-07-11T00:00:00+00:00", "count": 1}]
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        self.assertIsNone(workflow_cadence.workflow_count_at_or_after(snaps, when))

    def test_returns_the_earliest_at_or_after(self):
        snaps = [
            {"ts": "2026-07-11T00:00:00+00:00", "count": 1},
            {"ts": "2026-07-14T00:00:00+00:00", "count": 4},
            {"ts": "2026-07-16T00:00:00+00:00", "count": 5},
        ]
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        self.assertEqual(workflow_cadence.workflow_count_at_or_after(snaps, when), 4)
    ...
    def test_raises_tampered_error_on_a_malformed_line_instead_of_crashing(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        with self.assertRaises(workflow_cadence.WorkflowCadenceTamperedError):
            workflow_cadence.workflow_count_at_or_after(snaps, when)
...
class TestBuildPrediction(unittest.TestCase):
    def test_claim_names_the_threshold_and_target(self):
        ...
        self.assertEqual(payload["confidence"], workflow_cadence.DEFAULT_CONFIDENCE)
    ...
    def test_a_baseline_snapshot_names_the_real_delta(self):
        ...
        payload = workflow_cadence.build_prediction(_NOW, snaps, current_count=4, horizon_hours=336)
        self.assertIn("+1", payload["claim"])

    def test_claim_clears_copylint(self):
        payload = workflow_cadence.build_prediction(_NOW, [], current_count=4)
        result = copylint.enforce_copy(payload["claim"], payload["confidence"])
        self.assertTrue(result.ok)

    def test_rejects_naive_datetime(self):
        with self.assertRaises(workflow_cadence.WorkflowCadenceError):
            workflow_cadence.build_prediction(
                datetime.datetime(2026, 7, 20, 12, 0), [], current_count=4
            )

    def test_rejects_a_negative_count(self):
        with self.assertRaises(workflow_cadence.WorkflowCadenceError):
            workflow_cadence.build_prediction(_NOW, [], current_count=-1)
    ...
    def test_rejects_zero_horizon_hours(self):
        ...
        # be sealed.
        with self.assertRaises(workflow_cadence.WorkflowCadenceError):
            workflow_cadence.build_prediction(_NOW, [], current_count=1, horizon_hours=0)
...
class TestSealWorkflowPrediction(unittest.TestCase):
    def test_seals_a_real_predict_entry_to_a_scratch_ledger(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            entry = workflow_cadence.seal_workflow_prediction(
                now=_NOW,
            ...
            self.assertEqual(entry["actor"], "ogun")
            self.assertTrue(mod.verify())

    def test_a_tampered_sealed_workflow_prediction_breaks_verify(self):
        with tempfile.TemporaryDirectory() as tmp:
            ledger_path = os.path.join(tmp, "ledger.jsonl")
            mod = _fresh_ledger_module(ledger_path)
            workflow_cadence.seal_workflow_prediction(
                now=_NOW,
                ts=_NOW.isoformat(timespec="seconds"),
                current_count=4,
                snapshots=[],
