"""Task 39. The Oracle Desk's third real cadence: one checkable claim
about the town's own public GitHub fork count, sourced with no Arcade
scope at all (a repo's fork count is public data), copylint-clean,
...
from oracle_engine import copylint, fork_cadence, prediction  # noqa: E402
...
class TestFetchForkCount(unittest.TestCase):
    def test_reads_forks_count_off_the_injected_getter(self):
        def fake_get(url):
            self.assertEqual(url, "https://api.github.com/repos/thierrypdamiba/orita")
            return {"forks_count": 3}

        self.assertEqual(fork_cadence.fetch_fork_count(http_get=fake_get), 3)

    def test_rejects_a_malformed_response(self):
        with self.assertRaises(fork_cadence.ForkCadenceError):
            fork_cadence.fetch_fork_count(http_get=lambda url: {"nope": True})


class TestSnapshots(unittest.TestCase):
    def test_load_snapshots_of_a_missing_file_is_empty(self):
        self.assertEqual(fork_cadence.load_snapshots("/does/not/exist.jsonl"), [])

    def test_record_then_load_round_trips(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = os.path.join(tmp, "snapshots.jsonl")
            fork_cadence.record_snapshot(1, "2026-07-13T00:00:00+00:00", path=path)
            fork_cadence.record_snapshot(2, "2026-07-14T00:00:00+00:00", path=path)
            snaps = fork_cadence.load_snapshots(path)
    ...
    def test_never_rewrites_a_prior_line(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = os.path.join(tmp, "snapshots.jsonl")
            fork_cadence.record_snapshot(0, "2026-07-13T00:00:00+00:00", path=path)
            with open(path) as f:
                first_write = f.read()
            fork_cadence.record_snapshot(1, "2026-07-14T00:00:00+00:00", path=path)
            with open(path) as f:
                second_write = f.read()
...
class TestForkCountAtOrBefore(unittest.TestCase):
    def test_returns_none_with_no_early_enough_snapshot(self):
        ...
        self.assertIsNone(fork_cadence.fork_count_at_or_before(snaps, when))
    ...
    def test_a_valid_json_non_dict_snapshot_line_refuses_not_crashes(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            snaps = fork_cadence.load_snapshots(path)
            when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
            with self.assertRaises(fork_cadence.ForkCadenceTamperedError):
                fork_cadence.fork_count_at_or_before(snaps, when)
    ...
    def test_returns_the_latest_at_or_before(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        self.assertEqual(fork_cadence.fork_count_at_or_before(snaps, when), 1)
...
class TestForkCountAtOrAfter(unittest.TestCase):
    def test_returns_none_with_no_late_enough_snapshot(self):
        snaps = [{"ts": "2026-07-11T00:00:00+00:00", "count": 0}]
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        self.assertIsNone(fork_cadence.fork_count_at_or_after(snaps, when))
    ...
    def test_returns_the_earliest_at_or_after(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        self.assertEqual(fork_cadence.fork_count_at_or_after(snaps, when), 3)
    ...
    def test_raises_tampered_error_on_a_malformed_line_instead_of_crashing(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        with self.assertRaises(fork_cadence.ForkCadenceTamperedError):
            fork_cadence.fork_count_at_or_after(snaps, when)
...
class TestBuildPrediction(unittest.TestCase):
    def test_claim_names_the_threshold_and_target(self):
        payload = fork_cadence.build_prediction(_NOW, [], current_count=2, horizon_hours=336)
        self.assertIn("3", payload["claim"])
        self.assertIn("2026-08-03T12:00:00Z", payload["claim"])
        self.assertEqual(payload["confidence"], fork_cadence.DEFAULT_CONFIDENCE)
    ...
    def test_a_baseline_snapshot_names_the_real_delta(self):
        ...
        payload = fork_cadence.build_prediction(_NOW, snaps, current_count=2, horizon_hours=336)
        self.assertIn("+1", payload["claim"])

    def test_claim_clears_copylint(self):
        payload = fork_cadence.build_prediction(_NOW, [], current_count=2)
        result = copylint.enforce_copy(payload["claim"], payload["confidence"])
        self.assertTrue(result.ok)

    def test_rejects_naive_datetime(self):
        with self.assertRaises(fork_cadence.ForkCadenceError):
            fork_cadence.build_prediction(
                datetime.datetime(2026, 7, 20, 12, 0), [], current_count=2
            )

    def test_rejects_a_negative_count(self):
        with self.assertRaises(fork_cadence.ForkCadenceError):
            fork_cadence.build_prediction(_NOW, [], current_count=-1)
    ...
    def test_rejects_zero_horizon_hours(self):
        ...
        # be sealed.
        with self.assertRaises(fork_cadence.ForkCadenceError):
            fork_cadence.build_prediction(_NOW, [], current_count=1, horizon_hours=0)
    ...
    def test_rejects_negative_horizon_hours(self):
        ...
        with self.assertRaises(fork_cadence.ForkCadenceError):
            fork_cadence.build_prediction(_NOW, [], current_count=1, horizon_hours=-24)
    ...
    def test_non_utc_aware_now_still_targets_the_true_utc_instant(self):
        ...
        payload = fork_cadence.build_prediction(
            now_eastern, [], current_count=2, horizon_hours=336
        )
        self.assertIn("2026-08-03T12:00:00Z", payload["claim"])
...
class TestSealForkPrediction(unittest.TestCase):
    def test_seals_a_real_predict_entry_to_a_scratch_ledger(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            entry = fork_cadence.seal_fork_prediction(
                now=_NOW,
            ...
            self.assertEqual(entry["actor"], "kothar-wa-khasis")
            self.assertTrue(mod.verify())

    def test_a_tampered_sealed_fork_prediction_breaks_verify(self):
        with tempfile.TemporaryDirectory() as tmp:
            ledger_path = os.path.join(tmp, "ledger.jsonl")
            mod = _fresh_ledger_module(ledger_path)
            fork_cadence.seal_fork_prediction(
                now=_NOW,
                ts=_NOW.isoformat(timespec="seconds"),
                current_count=2,
                snapshots=[],
