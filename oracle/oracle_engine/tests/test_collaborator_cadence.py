...
class TestCollaboratorCountAtOrBefore(unittest.TestCase):
    ...
    def test_raises_tampered_error_on_a_malformed_line_instead_of_crashing(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        with self.assertRaises(collaborator_cadence.CollaboratorCadenceTamperedError):
            collaborator_cadence.collaborator_count_at_or_before(snaps, when)
...
class TestCollaboratorCountAtOrAfter(unittest.TestCase):
    ...
    def test_raises_tampered_error_on_a_malformed_line_instead_of_crashing(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        with self.assertRaises(collaborator_cadence.CollaboratorCadenceTamperedError):
            collaborator_cadence.collaborator_count_at_or_after(snaps, when)
...
class TestBuildPrediction(unittest.TestCase):
    ...
    def test_non_utc_aware_now_still_targets_the_true_utc_instant(self):
        ...
        payload = collaborator_cadence.build_prediction(now_eastern, [], current_count=4, horizon_hours=168)
    ...
    def test_utc_now_unaffected_by_the_normalization(self):
        ...
        payload = collaborator_cadence.build_prediction(_NOW, [], current_count=4, horizon_hours=168)
        self.assertIn("2026-07-27T12:00:00Z", payload["claim"])
