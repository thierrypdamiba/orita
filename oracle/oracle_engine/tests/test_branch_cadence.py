"""Task 48. The Oracle Desk's twelfth real cadence: one checkable claim
about the town's own public GitHub branch count, sourced with no Arcade
scope at all (a repo's branch list is public data), copylint-clean,
...
from oracle_engine import branch_cadence, copylint, prediction  # noqa: E402
...
class TestFetchBranchCount(unittest.TestCase):
    def test_reads_the_list_length_off_a_single_page(self):
        def fake_get(url):
            self.assertEqual(
                url,
                "https://api.github.com/repos/thierrypdamiba/orita/branches?per_page=100&page=1",
            )
            return [{"name": "main"}]

        self.assertEqual(branch_cadence.fetch_branch_count(http_get=fake_get), 1)
    ...
    def test_pages_until_a_short_page_ends_it(self):
        ...
        def fake_get(url):
            ...
            if page == 1:
                return [{"name": f"b{i}"} for i in range(100)]
            return [{"name": "b100"}, {"name": "b101"}]

        self.assertEqual(branch_cadence.fetch_branch_count(http_get=fake_get), 102)
        self.assertEqual(len(calls), 2)

    def test_rejects_a_malformed_response(self):
        with self.assertRaises(branch_cadence.BranchCadenceError):
            branch_cadence.fetch_branch_count(http_get=lambda url: {"nope": True})

    def test_empty_repo_is_zero(self):
        self.assertEqual(branch_cadence.fetch_branch_count(http_get=lambda url: []), 0)


class TestSnapshots(unittest.TestCase):
    def test_load_snapshots_of_a_missing_file_is_empty(self):
        self.assertEqual(branch_cadence.load_snapshots("/does/not/exist.jsonl"), [])

    def test_record_then_load_round_trips(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = os.path.join(tmp, "snapshots.jsonl")
            branch_cadence.record_snapshot(1, "2026-07-13T00:00:00+00:00", path=path)
            branch_cadence.record_snapshot(2, "2026-07-14T00:00:00+00:00", path=path)
            snaps = branch_cadence.load_snapshots(path)
...
class TestBranchCountAtOrBefore(unittest.TestCase):
    def test_returns_none_with_no_early_enough_snapshot(self):
        ...
        self.assertIsNone(branch_cadence.branch_count_at_or_before(snaps, when))
    ...
    def test_returns_the_latest_at_or_before(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        self.assertEqual(branch_cadence.branch_count_at_or_before(snaps, when), 1)
    ...
    def test_raises_tampered_error_on_a_malformed_line_instead_of_crashing(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        with self.assertRaises(branch_cadence.BranchCadenceTamperedError):
            branch_cadence.branch_count_at_or_before(snaps, when)

    def test_a_valid_json_non_dict_snapshot_line_refuses_not_crashes(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = os.path.join(tmp, "snapshots.jsonl")
            with open(path, "w", encoding="utf-8") as f:
                f.write('{"ts": "2026-07-11T00:00:00+00:00", "count": 1}\n')
                f.write("5\n")
            snaps = branch_cadence.load_snapshots(path)
            when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
            with self.assertRaises(branch_cadence.BranchCadenceTamperedError):
...
class TestBranchCountAtOrAfter(unittest.TestCase):
    def test_returns_none_with_no_late_enough_snapshot(self):
        snaps = [{"ts": "2026-07-11T00:00:00+00:00", "count": 1}]
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        self.assertIsNone(branch_cadence.branch_count_at_or_after(snaps, when))
    ...
    def test_returns_the_earliest_at_or_after(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        self.assertEqual(branch_cadence.branch_count_at_or_after(snaps, when), 2)
    ...
    def test_raises_tampered_error_on_a_malformed_line_instead_of_crashing(self):
        ...
        when = datetime.datetime(2026, 7, 13, tzinfo=datetime.timezone.utc)
        with self.assertRaises(branch_cadence.BranchCadenceTamperedError):
            branch_cadence.branch_count_at_or_after(snaps, when)
...
class TestBuildPrediction(unittest.TestCase):
    def test_claim_names_the_threshold_and_target(self):
        payload = branch_cadence.build_prediction(_NOW, [], current_count=1, horizon_hours=336)
        self.assertIn("2", payload["claim"])
        self.assertIn("2026-08-03T12:00:00Z", payload["claim"])
        self.assertEqual(payload["confidence"], branch_cadence.DEFAULT_CONFIDENCE)
    ...
    def test_a_baseline_snapshot_names_the_real_delta(self):
        ...
        payload = branch_cadence.build_prediction(_NOW, snaps, current_count=1, horizon_hours=336)
        self.assertIn("+0", payload["claim"])

    def test_claim_clears_copylint(self):
        payload = branch_cadence.build_prediction(_NOW, [], current_count=1)
        result = copylint.enforce_copy(payload["claim"], payload["confidence"])
        self.assertTrue(result.ok)

    def test_rejects_naive_datetime(self):
        with self.assertRaises(branch_cadence.BranchCadenceError):
            branch_cadence.build_prediction(
                datetime.datetime(2026, 7, 20, 12, 0), [], current_count=1
            )

    def test_rejects_a_negative_count(self):
        with self.assertRaises(branch_cadence.BranchCadenceError):
            branch_cadence.build_prediction(_NOW, [], current_count=-1)
    ...
    def test_rejects_zero_horizon_hours(self):
        ...
        # be sealed.
        with self.assertRaises(branch_cadence.BranchCadenceError):
            branch_cadence.build_prediction(_NOW, [], current_count=1, horizon_hours=0)
    ...
    def test_rejects_negative_horizon_hours(self):
        ...
        with self.assertRaises(branch_cadence.BranchCadenceError):
            branch_cadence.build_prediction(_NOW, [], current_count=1, horizon_hours=-24)
    ...
    def test_non_utc_aware_now_still_targets_the_true_utc_instant(self):
        ...
        payload = branch_cadence.build_prediction(now_eastern, [], current_count=4, horizon_hours=168)
    ...
    def test_utc_now_unaffected_by_the_normalization(self):
        ...
        payload = branch_cadence.build_prediction(_NOW, [], current_count=4, horizon_hours=168)
        self.assertIn("2026-07-27T12:00:00Z", payload["claim"])


class TestSealBranchPrediction(unittest.TestCase):
    def test_seals_a_real_predict_entry_to_a_scratch_ledger(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            entry = branch_cadence.seal_branch_prediction(
                now=_NOW,
    ...
    def test_a_tampered_sealed_branch_prediction_breaks_verify(self):
        with tempfile.TemporaryDirectory() as tmp:
            ledger_path = os.path.join(tmp, "ledger.jsonl")
            mod = _fresh_ledger_module(ledger_path)
            branch_cadence.seal_branch_prediction(
                now=_NOW,
                ts=_NOW.isoformat(timespec="seconds"),
                current_count=1,
                snapshots=[],
