...
A jaw-dropping, *safe*, live demo is what makes the platform worth forking. Fencepost is the sharpest and safest thing the town can ship, so it goes first. Later demos build on the same Arcade truth (the Oracle forecasting desk for a followable heartbeat — real and live since task 30, not the future maybe this sentence used to call it; see docs/oracle-desk.md for the running cadence-source count — which builds its research layer from the SAME PUBLIC PRIMITIVES Arcade Labs' financial-intelligence demo uses (Tavily MCP + Arcade) rather than forking that demo (it is public but UNLICENSED = all-rights-reserved; do not copy its code) and adds what is actually the gods' own: autonomous timestamped predictions, hash-chained self-scoring, and the public track-record narrative on top. Composition, not competition; published calls stay non-advice-shaped to avoid financial-advice risk). Anvil (a rollback/safety spine) is DROPPED: Fencepost is read-only so there is nothing to roll back, and Arcade already ships governance via arcade-guard + its audit product.
