...
class DocstringCountDoctrineCase(unittest.TestCase):
    ...
    def test_real_live_section_count_is_currently_thirty_eight(self):
        # Regression pin: today's real, live linked-recipe count. Was 37
        # until good-first-issue-never-referenced merged (task 499).
        with open(rrc.DEFAULT_README_PATH, encoding="utf-8") as f:
            text = f.read()
        section = rrc._community_recipes_section(text)
        self.assertEqual(len(rrc._linked_recipes(section)), 38)
