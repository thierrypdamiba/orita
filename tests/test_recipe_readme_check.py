...
class DocstringCountDoctrineCase(unittest.TestCase):
    ...
    def test_real_live_section_count_is_currently_forty_one(self):
        # Regression pin: today's real, live linked-recipe count. Was 40
        # until milestone-body-dangling-reference merged (task 520).
        with open(rrc.DEFAULT_README_PATH, encoding="utf-8") as f:
            text = f.read()
        section = rrc._community_recipes_section(text)
        self.assertEqual(len(rrc._linked_recipes(section)), 41)
