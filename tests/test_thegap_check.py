...
Task 1319 (Cluster Day, 2026-09-07): the fifth bug's confession came due
and was confessed unfound, and a sixth bug shipped the same hour, again
a brand-new function (`total_length()`). The real README now carries six
...
class RealVaultCase(unittest.TestCase):
    ...
    def test_real_live_readme_and_vault_fifth_bug_confessed_sixth_hidden(self):
        # Task 1319: the fifth bug's confession came due AND was posted
        # publicly the same hour (unfound), and a sixth bug was hidden the
        # same hour too (due 2026-09-14, not yet arrived under this
        # simulated `today`) -- so as of 2026-09-07 all five confessable
        # bugs read confessed and none is due, while the sixth's own
        # pre-draft is on record but not yet due (mirrors the fifth-hidden
        # state `test_real_live_readme_and_vault_fourth_bug_confessed_fifth_hidden`
        # already proved for its own generation).
        real_readme = os.path.join(ROOT, "thegap", "README.md")
        result = tgc.compute_cadence(real_readme, VAULT_ROOT, today=date(2026, 9, 7))
        self.assertEqual(result["missing_predraft"], [])
        self.assertEqual(result["confession_due_now"], [])
        self.assertEqual(
            result["confessed_on_record"],
