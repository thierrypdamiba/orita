...
*Task 1220 (2026-09-03): a live `Arcade_ListApps` recheck this hour (`tools/scopes_completeness_check.py` first caught the drift as "undocumented connected app(s): Github, X", which is what sent this table back to the source of truth) shows the-hand gateway now names its two connected apps `Github` and `X` — plain names, not the `arcade-github` / `ap_3GORxnS5T0YRHmzSRa0knq2nupY` opaque ids task 887 recorded. The permissions returned alongside each are byte-identical to what those two ids carried (`Github`: `["repo"]`; `X`: `["offline.access","tweet.read","tweet.write","users.read"]`) and `Arcade_ListApps`' own account field still names `thierrypdamiba` for GitHub — this reads as an upstream Arcade relabeling of the same two connections, not a new grant or a scope change. Corrected below rather than left for `scopes_completeness_check.py` to keep flagging a rename as a violation. `tools/arcade_app_watch.py record` holds this hour's raw read as the new baseline.*

| app_id | status |
|--|--|
| `Github` (was `arcade-github`) | in use by Fencepost, live-confirmed connected 2026-09-03, scope unchanged (`repo`), renamed upstream from `arcade-github` |
| `X` (was `ap_3GORxnS5T0YRHmzSRa0knq2nupY`) | in use by Fencepost, live-confirmed connected 2026-09-03, scope unchanged (`offline.access`, `tweet.read`, `tweet.write`, `users.read`), renamed upstream from the opaque X auth id |
| `arcade-google` | NOT connected as of 2026-09-03 (live `Arcade_ListApps` recheck; also absent 2026-08-20). Was never used by Fencepost (zero Gmail/Calendar-capable tools were ever exposed on the live gateway; see WIP note above). If it reconnects, `tools/gateway_toolset_check.py` still tracks the day a real Gmail/Calendar tool actually appears. |
| `arcade-linear` | NOT connected as of 2026-09-03 (live `Arcade_ListApps` recheck; also absent 2026-08-20). `RECIPES/linear-comment-claims-unfixed-issue/` (task 600) stays fixture-only; no live Linear-capable tool was ever exposed. |
| `arcade-slack` | NOT connected as of 2026-09-03 (live `Arcade_ListApps` recheck; also absent 2026-08-20). `RECIPES/slack-message-claims-unfixed-issue/` (task 599) stays fixture-only; no live Slack-capable tool was ever exposed. |
| `sybill` | NOT connected as of 2026-09-03 (live `Arcade_ListApps` recheck; also absent 2026-08-20). Was never used by Fencepost, no toolkit integration was ever planned. |
| `ap_3Fdn9uGDlF9Cj0ix9PJZbkMKDrF` (app management) | NOT connected as of 2026-09-03 (live `Arcade_ListApps` recheck; also absent 2026-08-20). Was never used by Fencepost, no toolkit integration was ever planned. |
