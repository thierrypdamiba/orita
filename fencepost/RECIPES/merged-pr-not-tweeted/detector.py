...
def _find_announcing_tweet(number: int, tweets: list[Tweet]) -> Tweet | None:
    ...
    # mentioned by a tweet naming the unrelated, longer "#13010". Shared
    # with issue-closed-not-tweeted's own identical seam via seam_engine.
    # mention_number (task 1362) rather than each retyping the same pattern.
    for tweet in tweets:
