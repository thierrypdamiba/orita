"""Tests for RECIPES/review-comment-claims-open-milestone/detector.py's own
detection logic (ROADMAP.md #588) -- the fifty-sixth real recipe: a pull
request's own inline code review comment claims a milestone shipped, but
the milestone never actually closed.
...
DETECTOR_PATH = FENCEPOST_ROOT / "RECIPES" / "review-comment-claims-open-milestone" / "detector.py"

_spec = importlib.util.spec_from_file_location("seam_engine._recipe_review_comment_claims_open_milestone_test", DETECTOR_PATH)
...
class TestClaimedMilestoneNumbers:
    ...
    def test_milestone_hash_n(self):
        assert detector._claimed_milestone_numbers("Milestone #6001 shipped.") == [6001]

    def test_case_insensitive(self):
        assert detector._claimed_milestone_numbers("MILESTONE #6002 is done.") == [6002]

    def test_bare_hash_n_with_no_milestone_word_is_never_extracted(self):
        assert detector._claimed_milestone_numbers("See #6003 for background.") == []

    def test_multiple_claims_in_one_body(self):
        assert detector._claimed_milestone_numbers("Milestone #6001. Milestone #6002.") == [6001, 6002]
    ...
    def test_duplicate_claim_in_one_body_is_not_deduplicated_at_extraction(self):
        ...
        assert detector._claimed_milestone_numbers("Milestone #6001 and milestone #6001 again.") == [6001, 6001]


class TestComputeGaps:
    def test_a_stale_open_claim_is_surfaced_at_high_confidence(self):
        comment = _comment(1, "This also ships milestone #6001 while we're in here.", _NOW - timedelta(hours=50))
        milestone = _milestone(6001, state="open")

        surfaced, excluded = detector.compute_gaps([comment], [milestone], now=_NOW)

        assert excluded == []
        assert len(surfaced) == 1
        assert surfaced[0].slug == "review-comment-claims-open-milestone-1-6001"
        assert surfaced[0].confidence == 0.85

    def test_a_fresh_open_claim_is_surfaced_at_low_confidence(self):
        comment = _comment(2, "I think this closes milestone #6003 too.", _NOW - timedelta(hours=4))
        milestone = _milestone(6003, state="open")
    ...
    def test_a_claim_at_exactly_the_edit_grace_window_boundary_is_high_confidence(self):
        comment = _comment(2, "Milestone #6003 shipped.", _NOW - timedelta(hours=24))
        milestone = _milestone(6003, state="open")
    ...
    def test_a_closed_claimed_milestone_is_excluded_not_surfaced(self):
        comment = _comment(3, "Milestone #6002 shipped as a side effect.", _NOW - timedelta(hours=50))
        milestone = _milestone(6002, state="closed")

        surfaced, excluded = detector.compute_gaps([comment], [milestone], now=_NOW)

        assert surfaced == []
        excluded_slugs = {g.slug for g in excluded}
        assert "claim-true-3-6002" in excluded_slugs
    ...
    def test_a_comment_with_no_claim_phrase_is_excluded_not_surfaced(self):
        comment = _comment(5, "Same root cause as #6005, not touching here.", _NOW - timedelta(hours=50))
    ...
    def test_a_duplicate_claim_in_one_comment_produces_one_candidate_not_two(self):
        comment = _comment(7, "Milestone #6001 shipped today and milestone #6001 confirmed again.", _NOW - timedelta(hours=3))
        milestone = _milestone(6001, state="open")

        surfaced, excluded = detector.compute_gaps([comment], [milestone], now=_NOW)

        assert len(surfaced) == 1
        assert surfaced[0].slug == "review-comment-claims-open-milestone-7-6001"

    def test_a_bare_hash_mention_with_no_milestone_word_does_not_clear_a_real_gap(self):
        comment = _comment(8, "Milestone #6001 shipped. See #6999 for background.", _NOW - timedelta(hours=50))
        milestone = _milestone(6001, state="open")

        surfaced, excluded = detector.compute_gaps([comment], [milestone], now=_NOW)

        assert len(surfaced) == 1
        assert surfaced[0].slug == "review-comment-claims-open-milestone-8-6001"
...
class TestRunRecipeScan:
    def test_the_shipped_fixture_elects_exactly_one_primary_gap(self):
        ...
        assert result["primary_gap"]["slug"] == "review-comment-claims-open-milestone-9301-6001"
        assert result["primary_gap"]["confidence"] == 0.85

    def test_the_shipped_fixture_weighs_two_fresh_claims_in_the_tail(self):
        result = detector.run_recipe_scan(now=_NOW)
        tail_slugs = {g["slug"] for g in result["tail"]}
        assert "review-comment-claims-open-milestone-9302-6003" in tail_slugs
        assert "review-comment-claims-open-milestone-9307-6001" in tail_slugs

    def test_the_shipped_fixture_excludes_the_true_dangling_and_no_claim_comments(self):
        result = detector.run_recipe_scan(now=_NOW)
        excluded_slugs = {g["slug"] for g in result["excluded"]}
        assert "claim-true-9303-6002" in excluded_slugs
        assert "claimed-milestone-not-found-9304-6999" in excluded_slugs
        assert "no-claim-phrase-9305" in excluded_slugs
    ...
    def test_the_shipped_fixtures_null_body_comment_produces_no_candidate(self):
        ...
        assert not any("-9306-" in s or s.endswith("-9306") for s in all_slugs)

    def test_the_shipped_fixture_deduplicates_the_repeated_6001_claim_within_one_comment(self):
        result = detector.run_recipe_scan(now=_NOW)
        all_slugs = [result["primary_gap"]["slug"]] if result["primary_gap"] else []
        all_slugs += [g["slug"] for g in result["tail"]]
        assert all_slugs.count("review-comment-claims-open-milestone-9307-6001") == 1


class TestLoaders:
    """load_review_comments/load_milestones -- mirrors every prior
    recipe's own _load_rows guard against syntactically valid but
    non-list JSON."""
