...
def test_real_plus_joined_counts_are_currently_fifteen_of_fifty_six():
    # Regression pin: today's real, live counts under RECIPES/. Was (15, 55)
    # until review-comment-claims-open-milestone merged (the fifty-sixth
    # real recipe, toolkit "github"-only -- only the total grows this time,
    # the same shape review-comment-claims-unfixed-issue's and review-
    # comment-claims-unmerged-pr's own merges already held.
    assert real_plus_joined_counts(FENCEPOST_ROOT) == (15, 56)
