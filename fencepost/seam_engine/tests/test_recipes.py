...
def test_real_plus_joined_counts_are_currently_fifteen_of_fifty_five():
    # Regression pin: today's real, live counts under RECIPES/. Was (15, 54)
    # until review-comment-claims-unmerged-pr merged (the fifty-fifth real
    # recipe, toolkit "github"-only -- only the total grows this time, the
    # same shape review-comment-claims-unfixed-issue's own merge held.
    assert real_plus_joined_counts(FENCEPOST_ROOT) == (15, 55)
