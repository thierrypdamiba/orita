...
def test_real_plus_joined_counts_are_currently_twenty_four_of_eighty_two():
    # Regression pin: today's real, live counts under RECIPES/. Was (24, 81)
    # until review-comment-claims-dangling-milestone merged (the
    # eighty-second real recipe, toolkit "github" -- single-toolkit, so the
    # plus-joined half stays put while the total climbs, the same shape
    # tag-never-released's own single-toolkit merge already showed.
    ...
    # pinning this number.
    assert real_plus_joined_counts(FENCEPOST_ROOT) == (24, 82)
