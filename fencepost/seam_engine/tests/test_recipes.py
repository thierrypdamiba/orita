...
def test_real_plus_joined_counts_are_currently_fifteen_of_sixty_two():
    # Regression pin: today's real, live counts under RECIPES/. Was (15, 61)
    # until commit-closes-keyword-issue-closed-not-planned merged (the
    # sixty-second real recipe, toolkit "github"-only -- only the total
    # grows this time, the same shape unblocked-issue-still-open's own
    # merge already held.
    assert real_plus_joined_counts(FENCEPOST_ROOT) == (15, 62)
