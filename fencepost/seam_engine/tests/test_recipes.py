...
# two-or-three separate words ("one hundred", "one hundred one") -- task
# 1195's own fix widened this from one word to two the first time either
# half of this claim ever needed a second word; this is the SECOND
# widening (two to three real words), the day the total count first
# needed a third (email-claims-dangling-milestone, the hundred-first real
# recipe).
_PLUS_JOINED_CLAIM_RE = re.compile(
    r"but ([a-z-]+) of the ([a-z]+(?:[ -][a-z]+){0,2}) real recipes today name a plus-joined pair"
)
...
def test_real_plus_joined_counts_are_currently_thirty_eight_of_one_hundred_eleven():
    ...
    # both counts climbed together that time: 38 of 106. Then
    # repo-description-claims-unfixed-issue merged (the hundred-seventh
    # real recipe, toolkit "github" -- NOT plus-joined), so only the total
    # climbed that time: 38 of 107. Then repo-description-claims-dangling-
    # milestone merged (the hundred-eighth real recipe, toolkit "github"
    ...
    # recipe, toolkit "github" -- NOT plus-joined), so the total climbed
    # again that time: 38 of 110. Then manifest-version-never-tagged
    # merged (the hundred-eleventh real recipe, toolkit "github" -- NOT
    # plus-joined), so only the total climbed again that time: 38 of 111.
    # Then merged-pr-changes-requested-not-addressed merged (the
    # hundred-twelfth real recipe, toolkit "github" -- NOT plus-joined),
    # so only the total climbed again that time: 38 of 112. Then
    # readme-star-count-stale merged (the hundred-thirteenth real recipe,
    # toolkit "github" -- NOT plus-joined), so only the total climbs again
    # this time: 38 of 113.
    ...
    # pinning this number.
    assert real_plus_joined_counts(FENCEPOST_ROOT) == (38, 113)
