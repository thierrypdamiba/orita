"""Tests for RECIPES/issue-comment-claims-unmerged-pr/detector.py's own
detection logic (ROADMAP.md #592) -- the sixtieth real recipe: an issue
or pull request's own ordinary timeline comment claims a PR shipped that
isn't merged.
...
DETECTOR_PATH = FENCEPOST_ROOT / "RECIPES" / "issue-comment-claims-unmerged-pr" / "detector.py"

_spec = importlib.util.spec_from_file_location("seam_engine._recipe_issue_comment_claims_unmerged_pr_test", DETECTOR_PATH)
detector = importlib.util.module_from_spec(_spec)
sys.modules[_spec.name] = detector
_spec.loader.exec_module(detector)

_NOW = datetime(2026, 8, 8, 0, 0, 0, tzinfo=timezone.utc)
...
def _pull(number: int, merged: bool = False, state: str = "open") -> "detector.PullRequest":
...
class TestClaimedPrNumbers:
    """Exercises the shared seam_engine.pr_claims grammar through this
    detector's own local alias -- the grammar itself is already covered
    exhaustively by test_pr_claims.py; this class only proves the alias
    is wired correctly."""

    def test_ships_hash_n(self):
        assert detector._claimed_pr_numbers("This ships #901.") == [901]

    def test_case_insensitive(self):
        assert detector._claimed_pr_numbers("SHIPS #902 is done.") == [902]

    def test_bare_hash_n_with_no_claim_word_is_never_extracted(self):
        assert detector._claimed_pr_numbers("See #903 for background.") == []
    ...
    def test_duplicate_claim_in_one_body_is_not_deduplicated_at_extraction(self):
        ...
        assert detector._claimed_pr_numbers("Ships #901 and ships #901 again.") == [901, 901]
...
class TestComputeGaps:
    def test_a_stale_open_claim_is_surfaced_at_high_confidence(self):
        ...
        pr = _pull(901, merged=False, state="open")

        surfaced, excluded = detector.compute_gaps([comment], [pr], now=_NOW)

        assert excluded == []
        assert len(surfaced) == 1
        assert surfaced[0].slug == "issue-comment-claims-unmerged-pr-1-901"
    ...
    def test_a_fresh_open_claim_is_surfaced_at_low_confidence(self):
        ...
        pr = _pull(903, merged=False, state="open")
    ...
    def test_a_claim_at_exactly_the_edit_grace_window_boundary_is_high_confidence(self):
        comment = _comment(2, "Ships #903.", _NOW - timedelta(hours=24))
        pr = _pull(903, merged=False, state="open")
    ...
    def test_a_merged_claimed_pr_is_excluded_not_surfaced(self):
        ...
        pr = _pull(902, merged=True, state="closed")
    ...
    def test_a_closed_unmerged_claimed_pr_is_still_surfaced(self):
        comment = _comment(9, "Ships #904, closed it out.", _NOW - timedelta(hours=50))
        pr = _pull(904, merged=False, state="closed")

        surfaced, excluded = detector.compute_gaps([comment], [pr], now=_NOW)

        assert excluded == []
        assert len(surfaced) == 1
        assert surfaced[0].slug == "issue-comment-claims-unmerged-pr-9-904"
    ...
    def test_a_duplicate_claim_in_one_comment_produces_one_candidate_not_two(self):
        comment = _comment(7, "Ships #901 today and via #901 confirmed again.", _NOW - timedelta(hours=3))
        pr = _pull(901, merged=False, state="open")

        surfaced, excluded = detector.compute_gaps([comment], [pr], now=_NOW)

        assert len(surfaced) == 1
        assert surfaced[0].slug == "issue-comment-claims-unmerged-pr-7-901"

    def test_a_claim_naming_a_number_with_no_matching_pr_in_an_empty_list_is_excluded(self):
        comment = _comment(8, "Ships #500, a number that isn't a real PR here.", _NOW - timedelta(hours=50))
...
class TestRunRecipeScan:
    def test_the_shipped_fixture_elects_exactly_one_primary_gap(self):
        ...
        assert result["primary_gap"]["slug"] == "issue-comment-claims-unmerged-pr-8201-901"
        assert result["primary_gap"]["confidence"] == 0.85

    def test_the_shipped_fixture_weighs_two_fresh_claims_in_the_tail(self):
        result = detector.run_recipe_scan(now=_NOW)
        tail_slugs = {g["slug"] for g in result["tail"]}
        assert "issue-comment-claims-unmerged-pr-8202-903" in tail_slugs
        assert "issue-comment-claims-unmerged-pr-8207-901" in tail_slugs

    def test_the_shipped_fixture_excludes_the_true_dangling_and_no_claim_comments(self):
        result = detector.run_recipe_scan(now=_NOW)
        excluded_slugs = {g["slug"] for g in result["excluded"]}
        assert "claim-true-8203-902" in excluded_slugs
        assert "claimed-pr-not-found-8204-999" in excluded_slugs
        assert "no-claim-phrase-8205" in excluded_slugs
    ...
    def test_the_shipped_fixtures_null_body_comment_produces_no_candidate(self):
        ...
        assert not any("-8206-" in s or s.endswith("-8206") for s in all_slugs)
    ...
    def test_the_shipped_fixture_deduplicates_the_repeated_901_claim_within_one_comment(self):
        ...
        all_slugs += [g["slug"] for g in result["tail"]]
        assert all_slugs.count("issue-comment-claims-unmerged-pr-8207-901") == 1


class TestLoaders:
    """load_issue_comments/load_pulls -- mirrors every prior recipe's own
