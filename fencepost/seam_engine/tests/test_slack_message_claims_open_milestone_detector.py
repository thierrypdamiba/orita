"""Tests for RECIPES/slack-message-claims-open-milestone/detector.py's own
detection logic -- the sixty-ninth real recipe: a Slack channel message
claims a milestone shipped, but the named milestone never actually
closed. The milestone-claim twin of
test_slack_message_claims_unfixed_issue_detector.py, and the Slack-side
twin of test_mention_claims_open_milestone_detector.py.
...
DETECTOR_PATH = FENCEPOST_ROOT / "RECIPES" / "slack-message-claims-open-milestone" / "detector.py"

_spec = importlib.util.spec_from_file_location("seam_engine._recipe_slack_message_claims_open_milestone_test", DETECTOR_PATH)
...
class TestClaimedMilestoneNumbers:
    def test_milestone_hash_n(self):
        assert detector._claimed_milestone_numbers("Milestone #4301 shipped.") == [4301]

    def test_case_insensitive(self):
        assert detector._claimed_milestone_numbers("MILESTONE #4302 done.") == [4302]

    def test_bare_hash_n_with_no_milestone_word_is_never_extracted(self):
        assert detector._claimed_milestone_numbers("See #4303 for background.") == []

    def test_multiple_claims_in_one_message(self):
        assert detector._claimed_milestone_numbers("Milestone #4301. Milestone #4302.") == [4301, 4302]
    ...
    def test_duplicate_claim_in_one_message_is_not_deduplicated_at_extraction(self):
        ...
        assert detector._claimed_milestone_numbers("Milestone #4301 and milestone #4301 again.") == [4301, 4301]


class TestComputeGaps:
    def test_a_stale_open_claim_is_surfaced_at_high_confidence(self):
        message = _message("S-1", "Milestone #4301 shipped.", _NOW - timedelta(hours=50))
        milestone = _milestone(4301, state="open")

        surfaced, excluded = detector.compute_gaps([message], [milestone], now=_NOW)

        assert excluded == []
        assert len(surfaced) == 1
        assert surfaced[0].slug == "slack-message-claims-open-milestone-S-1-4301"
        assert surfaced[0].confidence == 0.85

    def test_a_fresh_open_claim_is_surfaced_at_low_confidence(self):
        message = _message("S-2", "Milestone #4303 done.", _NOW - timedelta(hours=4))
        milestone = _milestone(4303, state="open")
    ...
    def test_a_closed_claimed_milestone_is_excluded_not_surfaced(self):
        message = _message("S-3", "Milestone #4302 wrapped up.", _NOW - timedelta(hours=50))
        milestone = _milestone(4302, state="closed")

        surfaced, excluded = detector.compute_gaps([message], [milestone], now=_NOW)

        assert surfaced == []
        excluded_slugs = {g.slug for g in excluded}
        assert "claim-true-S-3-4302" in excluded_slugs

    def test_a_claim_naming_a_milestone_that_does_not_exist_is_excluded_not_surfaced(self):
        message = _message("S-4", "Milestone #4999 shipped today.", _NOW - timedelta(hours=50))

        surfaced, excluded = detector.compute_gaps([message], [], now=_NOW)

        assert surfaced == []
        excluded_slugs = {g.slug for g in excluded}
        assert "claimed-milestone-not-found-S-4-4999" in excluded_slugs

    def test_a_message_with_no_claim_phrase_produces_no_candidate_at_all(self):
        message = _message("S-5", "Housekeeping only, see #4105 for background.", _NOW - timedelta(hours=50))
    ...
    def test_a_duplicate_claim_in_one_message_produces_one_candidate_not_two(self):
        message = _message("S-6", "Milestone #4301 shipped and milestone #4301 confirmed again.", _NOW - timedelta(hours=50))
        milestone = _milestone(4301, state="open")

        surfaced, excluded = detector.compute_gaps([message], [milestone], now=_NOW)

        assert len(surfaced) == 1
        assert surfaced[0].slug == "slack-message-claims-open-milestone-S-6-4301"

    def test_confidence_boundary_exactly_24_hours_is_stale_not_fresh(self):
        message = _message("S-7", "Milestone #4301 shipped.", _NOW - timedelta(hours=24))
        milestone = _milestone(4301, state="open")
    ...
    def test_confidence_boundary_just_under_24_hours_is_fresh(self):
        message = _message("S-8", "Milestone #4301 shipped.", _NOW - timedelta(hours=23, minutes=59))
        milestone = _milestone(4301, state="open")
    ...
    def test_evidence_carries_both_the_message_url_and_the_milestone_url(self):
        message = _message("S-9", "Milestone #4301 shipped.", _NOW - timedelta(hours=50))
        milestone = _milestone(4301, state="open")

        surfaced, _excluded = detector.compute_gaps([message], [milestone], now=_NOW)

        assert surfaced[0].evidence == [message.url, milestone.url]

    def test_excluded_not_found_evidence_carries_only_the_message_url(self):
        message = _message("S-10", "Milestone #4999 shipped today.", _NOW - timedelta(hours=50))
    ...
    def test_no_surfaced_or_excluded_candidates_for_an_empty_message_list(self):
        surfaced, excluded = detector.compute_gaps([], [_milestone(4301)], now=_NOW)
        assert surfaced == []
        assert excluded == []

    def test_surfaced_candidates_are_sorted_by_confidence_descending(self):
        stale = _message("S-11", "Milestone #4301 shipped.", _NOW - timedelta(hours=50))
        fresh = _message("S-12", "Milestone #4303 shipped.", _NOW - timedelta(hours=4))
        milestones = [_milestone(4301, state="open"), _milestone(4303, state="open")]
    ...
    def test_headline_and_detail_never_grade_or_blame_the_poster(self):
        ...
        message = _message("S-13", "Milestone #4301 shipped.", _NOW - timedelta(hours=50), author="some-mortal")
        milestone = _milestone(4301, state="open")
...
class TestRunRecipeScan:
    def test_the_shipped_fixture_elects_exactly_one_primary_gap(self):
        ...
        assert result["primary_gap"]["slug"] == "slack-message-claims-open-milestone-SLK-4101-4301"
        assert result["primary_gap"]["confidence"] == 0.85

    def test_the_shipped_fixture_weighs_the_fresh_claim_in_the_tail(self):
        result = detector.run_recipe_scan(now=_NOW)
        tail_slugs = {g["slug"] for g in result["tail"]}
        assert "slack-message-claims-open-milestone-SLK-4102-4303" in tail_slugs

    def test_the_shipped_fixture_excludes_the_true_claim_and_the_not_found_and_no_claim_messages(self):
        result = detector.run_recipe_scan(now=_NOW)
        excluded_slugs = {g["slug"] for g in result["excluded"]}
        assert "claim-true-SLK-4103-4302" in excluded_slugs
        assert "claimed-milestone-not-found-SLK-4104-4999" in excluded_slugs
        assert "no-claim-phrase-SLK-4105" in excluded_slugs

    def test_the_shipped_fixture_deduplicates_the_repeated_4301_claim(self):
        result = detector.run_recipe_scan(now=_NOW)
        all_slugs = [result["primary_gap"]["slug"]] if result["primary_gap"] else []
        all_slugs += [g["slug"] for g in result["tail"]]
        assert all_slugs.count("slack-message-claims-open-milestone-SLK-4101-4301") == 1
...
class TestManifest:
    ...
    def test_recipe_json_clears_validate_recipe(self):
        ...
        manifest_path = FENCEPOST_ROOT / "RECIPES" / "slack-message-claims-open-milestone" / "recipe.json"
        manifest = load_recipe_manifest(manifest_path)
        validated = validate_recipe(manifest)
        assert validated.slug == "slack-message-claims-open-milestone"
        assert validated.toolkit == "slack+github"
        assert set(validated.scopes) == {"SearchChannelMessages", "ListMilestones"}
