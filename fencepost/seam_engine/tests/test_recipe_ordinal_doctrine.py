...
def test_the_real_ordinal_claims_are_currently_clean():
    """Regression pin against today's real, live tree: 48 real recipes,
    the reference recipe unnumbered, the other 47 forming the unbroken
    sequence 2..48 -- no duplicate, no gap. Was 47/46/2..47 until
    mention-claims-open-milestone merged (the forty-eighth real recipe)."""
    claims = live_ordinal_claims(FENCEPOST_ROOT)
    numbered = sorted(n for n in claims.values() if n is not None)
    assert len(claims) == 48
    assert numbered == list(range(2, 49))
