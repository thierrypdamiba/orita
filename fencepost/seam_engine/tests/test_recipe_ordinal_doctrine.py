...
def test_the_real_ordinal_claims_are_currently_clean():
    """Regression pin against today's real, live tree: 71 real recipes,
    the reference recipe unnumbered, the other 70 forming the unbroken
    sequence 2..71 -- no duplicate, no gap. Was 70/69/2..70 until
    linear-comment-claims-unmerged-pr merged (the seventy-first real
    recipe)."""
    claims = live_ordinal_claims(FENCEPOST_ROOT)
    numbered = sorted(n for n in claims.values() if n is not None)
    assert len(claims) == 71
    assert numbered == list(range(2, 72))
