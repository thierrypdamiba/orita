...
def test_the_real_ordinal_claims_are_currently_clean():
    """Regression pin against today's real, live tree: 66 real recipes,
    the reference recipe unnumbered, the other 65 forming the unbroken
    sequence 2..66 -- no duplicate, no gap. Was 65/64/2..65 until
    commit-claims-open-milestone merged (the sixty-sixth real
    recipe)."""
    claims = live_ordinal_claims(FENCEPOST_ROOT)
    numbered = sorted(n for n in claims.values() if n is not None)
    assert len(claims) == 66
    assert numbered == list(range(2, 67))
