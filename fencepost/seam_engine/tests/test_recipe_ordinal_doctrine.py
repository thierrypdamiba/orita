...
def test_the_real_ordinal_claims_are_currently_clean():
    """Regression pin against today's real, live tree: 45 real recipes,
    the reference recipe unnumbered, the other 44 forming the unbroken
    sequence 2..45 -- no duplicate, no gap. Was 44/43/2..44 until
    milestone-claims-unfixed-issue merged (ROADMAP.md #535)."""
    claims = live_ordinal_claims(FENCEPOST_ROOT)
    numbered = sorted(n for n in claims.values() if n is not None)
    assert len(claims) == 45
    assert numbered == list(range(2, 46))
