...
def test_the_real_ordinal_claims_are_currently_clean():
    """Regression pin against today's real, live tree: 57 real recipes,
    the reference recipe unnumbered, the other 56 forming the unbroken
    sequence 2..57 -- no duplicate, no gap. Was 56/55/2..56 until
    readme-dangling-reference merged (the fifty-seventh real
    recipe)."""
    claims = live_ordinal_claims(FENCEPOST_ROOT)
    numbered = sorted(n for n in claims.values() if n is not None)
    assert len(claims) == 57
    assert numbered == list(range(2, 58))
