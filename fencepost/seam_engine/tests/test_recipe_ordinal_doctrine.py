...
def test_the_real_ordinal_claims_are_currently_clean():
    """Regression pin against today's real, live tree: 80 real recipes,
    the reference recipe unnumbered, the other 79 forming the unbroken
    sequence 2..80 -- no duplicate, no gap. Was 79/78/2..79 until
    repo-description-dangling-reference merged (the eightieth real
    recipe)."""
    claims = live_ordinal_claims(FENCEPOST_ROOT)
    numbered = sorted(n for n in claims.values() if n is not None)
    assert len(claims) == 80
    assert numbered == list(range(2, 81))
