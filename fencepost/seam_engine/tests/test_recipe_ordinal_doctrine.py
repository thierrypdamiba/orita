...
def test_the_real_ordinal_claims_are_currently_clean():
    """Regression pin against today's real, live tree: 72 real recipes,
    the reference recipe unnumbered, the other 71 forming the unbroken
    sequence 2..72 -- no duplicate, no gap. Was 71/70/2..71 until
    slack-message-claims-unmerged-pr merged (the seventy-second real
    recipe)."""
    claims = live_ordinal_claims(FENCEPOST_ROOT)
    numbered = sorted(n for n in claims.values() if n is not None)
    assert len(claims) == 72
    assert numbered == list(range(2, 73))
