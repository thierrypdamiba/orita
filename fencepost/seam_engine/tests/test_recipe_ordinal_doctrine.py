...
_ORDINAL_WORDS = {
    ...
    "fifty-seventh": 57, "fifty-eighth": 58,
}
...
def test_the_real_ordinal_claims_are_currently_clean():
    """Regression pin against today's real, live tree: 58 real recipes,
    the reference recipe unnumbered, the other 57 forming the unbroken
    sequence 2..58 -- no duplicate, no gap. Was 57/56/2..57 until
    issue-comment-claims-unfixed-issue merged (the fifty-eighth real
    recipe)."""
    claims = live_ordinal_claims(FENCEPOST_ROOT)
    numbered = sorted(n for n in claims.values() if n is not None)
    assert len(claims) == 58
    assert numbered == list(range(2, 59))
