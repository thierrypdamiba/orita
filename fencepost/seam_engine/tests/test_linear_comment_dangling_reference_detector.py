"""Tests for RECIPES/linear-comment-dangling-reference/detector.py's own
detection logic -- the seventy-fourth real recipe, and the ninth leg of
...
issue/PR timeline; this one watches a comment left on a Linear issue, the
first non-GitHub/X toolkit this family has touched).
...
copy -- same discipline as `test_issue_comment_dangling_reference_detector.py`
...
DETECTOR_PATH = FENCEPOST_ROOT / "RECIPES" / "linear-comment-dangling-reference" / "detector.py"

_spec = importlib.util.spec_from_file_location("seam_engine._recipe_linear_comment_dangling_reference_test", DETECTOR_PATH)
...
def _comment(
    id: str, text: str | None, *, issue_identifier: str = "ENG-1", hours_ago: float = 100.0,
) -> "detector.Comment":
    created = _NOW.timestamp() - hours_ago * 3600
    return detector.Comment(
        id=id, issue_identifier=issue_identifier, author="some-teammate",
        text=text or "",
        created_at=datetime.fromtimestamp(created, tz=timezone.utc),
        url=f"https://linear.app/orita-town/issue/{issue_identifier}#comment-{id}",
    )
...
class TestComputeGaps:
    def test_a_reference_to_a_nonexistent_number_is_surfaced(self):
        comments = [_comment("c1", "see #99 for context", hours_ago=100.0)]
        surfaced, excluded = detector.compute_gaps(comments, [], [], now=_NOW)

        assert excluded == []
        assert len(surfaced) == 1
        assert surfaced[0].slug == "linear-comment-dangling-reference-c1-99"
        assert surfaced[0].confidence == 0.85

    def test_a_fresh_dangling_reference_scores_the_lower_bar(self):
        comments = [_comment("c1", "see #99 for context", hours_ago=2.0)]
        surfaced, excluded = detector.compute_gaps(comments, [], [], now=_NOW)
    ...
    def test_a_reference_matching_a_real_issue_is_excluded(self):
        comments = [_comment("c1", "same root cause as #501")]
    ...
    def test_a_reference_matching_a_real_pr_is_excluded(self):
        comments = [_comment("c1", "waiting on #510 to land")]
    ...
    def test_a_comment_with_no_body_is_excluded_outright(self):
        comments = [_comment("c1", None, hours_ago=100.0)]
        surfaced, excluded = detector.compute_gaps(comments, [], [], now=_NOW)
    ...
    def test_a_comment_with_no_reference_produces_no_candidates(self):
        comments = [_comment("c1", "just a thank-you, nothing to track", hours_ago=100.0)]
        surfaced, excluded = detector.compute_gaps(comments, [], [], now=_NOW)
    ...
    def test_a_cross_repo_reference_is_never_extracted(self):
        comments = [_comment("c1", "saw arcadeai/gasstation#42, worth a look", hours_ago=100.0)]
        surfaced, excluded = detector.compute_gaps(comments, [], [], now=_NOW)
    ...
    def test_a_repeated_reference_in_one_comment_is_deduped(self):
        comments = [_comment("c1", "see #99 and also #99 again", hours_ago=100.0)]
    ...
    def test_surfaced_candidates_sort_by_confidence_descending(self):
        comments = [
            _comment("c1", "see #10", hours_ago=2.0),   # 0.55
            _comment("c2", "see #20", hours_ago=100.0),  # 0.85
        ]
...
class TestRunRecipeScanAgainstOwnFixture:
    def test_the_shipped_fixture_elects_one_primary_gap(self):
        ...
        assert result["primary_gap"]["slug"] == "linear-comment-dangling-reference-LIN-C-9001-4299"
    ...
    def test_the_shipped_fixture_weighs_one_coincidence_in_the_tail(self):
        ...
        assert "linear-comment-dangling-reference-LIN-C-9002-4298" in tail_slugs

    def test_the_shipped_fixture_excludes_the_real_issue_and_pr_matches(self):
        result = detector.run_recipe_scan(now=_NOW)

        excluded_slugs = [g["slug"] for g in result["excluded"]]
        assert "linear-comment-ref-matched-LIN-C-9003-4210" in excluded_slugs
        assert "linear-comment-ref-matched-LIN-C-9004-4220" in excluded_slugs
