"""Tests for RECIPES/calendar-event-dangling-reference/detector.py's own
detection logic -- the hundred-sixth real recipe, and the twelfth leg of
...
to Slack, email-dangling-reference watches an inbound email; this one
watches a Google Calendar event).
...
copy -- same discipline as `test_email_dangling_reference_detector.py`
...
DETECTOR_PATH = FENCEPOST_ROOT / "RECIPES" / "calendar-event-dangling-reference" / "detector.py"

_spec = importlib.util.spec_from_file_location("seam_engine._recipe_calendar_event_dangling_reference_test", DETECTOR_PATH)
...
def _event(id: str, title: str, description: str | None = None, *, organizer: str = "some-organizer@example.com") -> "detector.CalendarEvent":
    return detector.CalendarEvent(
        id=id, title=title, description=description or "", organizer=organizer,
...
class TestComputeGaps:
    def test_a_reference_to_a_nonexistent_number_is_surfaced(self):
        events = [_event("e1", "sync", "see #99 for context")]
        surfaced, excluded = detector.compute_gaps(events, [], [], now=_NOW)

        assert excluded == []
        assert len(surfaced) == 1
        assert surfaced[0].slug == "calendar-event-dangling-reference-e1-99"
        assert surfaced[0].confidence == 0.75

    def test_a_reference_matching_a_real_issue_is_excluded(self):
        events = [_event("e1", "retro", "same root cause as #501")]
        surfaced, excluded = detector.compute_gaps(events, [_issue(501)], [], now=_NOW)
    ...
    def test_a_reference_matching_a_real_pr_is_excluded(self):
        events = [_event("e1", "office hours", "waiting on #510 to land")]
        surfaced, excluded = detector.compute_gaps(events, [], [_pull(510)], now=_NOW)

        assert surfaced == []
        assert len(excluded) == 1
        assert excluded[0].confidence == 0.0

    def test_an_event_with_no_title_or_description_reference_is_excluded_outright(self):
        events = [_event("e1", "standup", None)]
        surfaced, excluded = detector.compute_gaps(events, [], [], now=_NOW)

        assert surfaced == []
        assert excluded == []

    def test_an_event_with_no_reference_produces_no_candidates(self):
        events = [_event("e1", "weekly standup", "quiet week, nothing to flag")]
        surfaced, excluded = detector.compute_gaps(events, [], [], now=_NOW)
    ...
    def test_a_cross_repo_reference_is_never_extracted(self):
        events = [_event("e1", "office hours", "saw arcadeai/gasstation#42, worth a look")]
        surfaced, excluded = detector.compute_gaps(events, [], [], now=_NOW)

        assert surfaced == []
        assert excluded == []

    def test_a_repeated_reference_in_one_event_is_deduped(self):
        events = [_event("e1", "see #99", "also #99 again")]
        surfaced, _excluded = detector.compute_gaps(events, [], [], now=_NOW)

        assert len(surfaced) == 1

    def test_a_reference_in_the_title_is_found_same_as_the_description(self):
        events = [_event("e1", "streak review -- #4103 got merged already right?", "circling back")]
        surfaced, excluded = detector.compute_gaps(events, [], [_pull(4103)], now=_NOW)

        assert surfaced == []
        assert len(excluded) == 1

    def test_all_surfaced_candidates_share_the_flat_confidence(self):
        events = [_event("e1", "see #10"), _event("e2", "see #20")]
        surfaced, _ = detector.compute_gaps(events, [], [], now=_NOW)
...
class TestRunRecipeScanAgainstOwnFixture:
    def test_the_shipped_fixture_elects_one_primary_gap(self):
        ...
        assert result["primary_gap"]["slug"] == "calendar-event-dangling-reference-EVT-R-4801-4501"
    ...
    def test_the_shipped_fixture_excludes_the_real_issue_and_pr_matches(self):
        ...
        assert "calendar-ref-matched-EVT-R-4802-4102" in excluded_slugs
        assert "calendar-ref-matched-EVT-R-4805-4103" in excluded_slugs
