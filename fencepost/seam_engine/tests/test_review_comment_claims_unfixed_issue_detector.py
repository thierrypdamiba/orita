"""Tests for RECIPES/review-comment-claims-unfixed-issue/detector.py's own
detection logic -- the fifty-fourth real recipe: a pull request's own
inline code review comment invokes a real closing keyword against an
...
DETECTOR_PATH = FENCEPOST_ROOT / "RECIPES" / "review-comment-claims-unfixed-issue" / "detector.py"

_spec = importlib.util.spec_from_file_location("seam_engine._recipe_review_comment_claims_unfixed_issue_test", DETECTOR_PATH)
...
class TestClaimedIssueNumbers:
    def test_fixes_hash_n(self):
        assert detector._claimed_issue_numbers("This also fixes #2101.") == [2101]

    def test_closes_hash_n(self):
        assert detector._claimed_issue_numbers("Closes #2102 as a side effect.") == [2102]

    def test_resolves_hash_n(self):
        assert detector._claimed_issue_numbers("I think this resolves #2103 too.") == [2103]

    def test_past_tense_fixed_hash_n(self):
        assert detector._claimed_issue_numbers("Fixed #2104 in the last commit.") == [2104]
    ...
    def test_bare_hash_n_with_no_claim_verb_is_never_extracted(self):
        # This is issue-body-dangling-reference's/review-comment-dangling-
        # reference's own broader regex; a bare "same root cause as #N"
        # aside is not a fixed-it claim.
        assert detector._claimed_issue_numbers("Same root cause as #2107, not touching here.") == []

    def test_multiple_claims_in_one_comment(self):
        assert detector._claimed_issue_numbers("Fixes #2101. Also closes #2102.") == [2101, 2102]
    ...
    def test_duplicate_claim_in_one_comment_is_not_deduplicated_at_extraction(self):
        ...
        assert detector._claimed_issue_numbers("Fixes #2101 -- yes, really closes #2101.") == [2101, 2101]


class TestComputeGaps:
    def test_a_stale_unfixed_claim_is_surfaced_at_high_confidence(self):
        comment = _comment(1, "This also fixes #2101 while we're in here.", _NOW - timedelta(hours=50))
        ...
        assert surfaced[0].slug == "review-comment-claims-unfixed-issue-1-2101"
        assert surfaced[0].confidence == 0.85

    def test_a_fresh_unfixed_claim_is_surfaced_at_low_confidence(self):
        comment = _comment(2, "I think this resolves #2103 too.", _NOW - timedelta(hours=4))
    ...
    def test_a_claim_at_exactly_the_edit_grace_window_boundary_is_high_confidence(self):
        comment = _comment(2, "Closes #2103.", _NOW - timedelta(hours=24))
        issue = _issue(2103, state="open")
    ...
    def test_a_closed_claimed_issue_is_excluded_not_surfaced(self):
        comment = _comment(3, "Closes #2102 as a side effect.", _NOW - timedelta(hours=50))
        ...
        assert "claim-true-3-2102" in excluded_slugs
    ...
    def test_a_comment_with_no_claim_phrase_is_excluded_not_surfaced(self):
        comment = _comment(5, "Same root cause as #2105, not touching here.", _NOW - timedelta(hours=50))
    ...
    def test_a_duplicate_claim_in_one_comment_produces_one_candidate_not_two(self):
        comment = _comment(7, "Fixes #2101 -- yes, really closes #2101.", _NOW - timedelta(hours=3))
        issue = _issue(2101, state="open")

        surfaced, excluded = detector.compute_gaps([comment], [issue], now=_NOW)

        assert len(surfaced) == 1
        assert surfaced[0].slug == "review-comment-claims-unfixed-issue-7-2101"
...
class TestRunRecipeScan:
    def test_the_shipped_fixture_elects_exactly_one_primary_gap(self):
        ...
        assert result["primary_gap"]["slug"] == "review-comment-claims-unfixed-issue-9101-2101"
        assert result["primary_gap"]["confidence"] == 0.85

    def test_the_shipped_fixture_weighs_two_fresh_claims_in_the_tail(self):
        result = detector.run_recipe_scan(now=_NOW)
        tail_slugs = {g["slug"] for g in result["tail"]}
        assert "review-comment-claims-unfixed-issue-9102-2103" in tail_slugs
        assert "review-comment-claims-unfixed-issue-9107-2101" in tail_slugs

    def test_the_shipped_fixture_excludes_the_true_dangling_and_no_claim_comments(self):
        result = detector.run_recipe_scan(now=_NOW)
        excluded_slugs = {g["slug"] for g in result["excluded"]}
        assert "claim-true-9103-2102" in excluded_slugs
        assert "claimed-issue-not-found-9104-2999" in excluded_slugs
        assert "no-claim-phrase-9105" in excluded_slugs
    ...
    def test_the_shipped_fixtures_null_body_comment_produces_no_candidate(self):
        ...
        assert not any("-9106-" in s or s.endswith("-9106") for s in all_slugs)

    def test_the_shipped_fixture_deduplicates_the_repeated_2101_claim_within_one_comment(self):
        result = detector.run_recipe_scan(now=_NOW)
        all_slugs = [result["primary_gap"]["slug"]] if result["primary_gap"] else []
        all_slugs += [g["slug"] for g in result["tail"]]
        assert all_slugs.count("review-comment-claims-unfixed-issue-9107-2101") == 1


class TestLoaders:
    """load_review_comments/load_issues -- mirrors every prior recipe's
