...
def test_runs_all_real_recipes_in_the_actual_repo_without_error(monkeypatch):
    # The integration proof: against the real RECIPES/ tree (all eight real
    # recipes shipped by tasks 22, 108, 110, 368, 371, 373, 376, 377), the combined scan runs clean.
    monkeypatch.setattr(scan_mod, "fetch_github_activity", lambda *a, **k: [])
