...
class TestClosedMilestoneWithNoTimestampIsNotMislabeledStillOpen:
    ...
    def test_a_closed_milestone_with_no_timestamp_is_excluded_as_malformed_not_still_open(self):
        milestone = _milestone(760, state="closed", closed_at=None)
        issue = _issue(761, milestone_number=760, state="open")

        surfaced, excluded = detector.compute_gaps([milestone], [issue], now=_NOW)

        assert surfaced == []
        by_slug = {g.slug: g for g in excluded}
        candidate = by_slug["milestone-closed-no-timestamp-761-760"]
        assert "has not closed yet" not in candidate.detail
        assert "malformed" in candidate.detail
        assert candidate.evidence == [issue.url, milestone.url]
