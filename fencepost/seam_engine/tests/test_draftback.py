...
def test_main_with_file_arg_raises_named_error_not_attributeerror_when_json_is_not_an_object(tmp_path: Path):
    # main()'s file-arg branch used to do `_load_sealed(path)` ->
    # `json.loads(path.read_text())` with no shape check, then hand the
    # parsed value straight to `render_email_draft`/`render_notion_page`,
    # which open with `sealed.get(...)`. A CLI-supplied file can be any
    # syntactically valid JSON -- a bare list, int, bool, null, or string,
    # not just an object -- so `[1, 2, 3]` on disk crashed this with a bare
    ...
    # `_load_sealed_arg` and `ledger.py`'s `_load_scan` already hold in
    # this package.
...
def test_main_with_stdin_raises_named_error_not_attributeerror_when_json_is_not_an_object(tmp_path: Path, monkeypatch):
    # Same bug, reached through the CLI's stdin branch (`argv == [channel, "-"]`).
    import io
    import sys
