...
def test_real_recipe_count_is_currently_thirty_seven():
    """Regression pin: today's real, live count under RECIPES/. Was 36 until
    RECIPES/readme-claims-unmerged-pr/ merged (the thirty-seventh real
    recipe, task 493) -- the exact drift this whole doctrine file exists to
    catch, now caught once for real instead of only rehearsed by the
    mutation test below."""
    assert real_recipe_count(FENCEPOST_ROOT) == 37
