...
# task 1195's own fix widened this from one word to two the first time the
# claim ever needed a second word; this is the SECOND widening (two to
# three real words), the day the count first needed a third
# (email-claims-dangling-milestone, the hundred-first real recipe): every
# count through 99 fits one hyphenated token, 100 is "one hundred" (two
# words), and 101 is "one hundred one" (three words) in ordinary English
# prose. The old two-word-max shape could never have matched a third word
# at all.
_CLAIM_RE = re.compile(r"([A-Za-z]+(?:[ -][A-Za-z]+){0,2}) real recipes stand today")
...
def test_real_recipe_count_is_currently_one_hundred_eleven():
    """Regression pin: today's real, live count under RECIPES/. Was 105
    until RECIPES/calendar-event-dangling-reference/ merged (the
    hundred-sixth real recipe), then 106 until RECIPES/repo-description-
    claims-unfixed-issue/ merged (the hundred-seventh), then 107 until
    RECIPES/repo-description-claims-dangling-milestone/ merged (the
    hundred-eighth), then 108 until RECIPES/repo-description-claims-open-
    ...
    not-addressed/ merged (the hundred-twelfth), then 112 until
    RECIPES/readme-star-count-stale/ merged (the hundred-thirteenth) -- the
    exact drift this whole doctrine file exists to catch, now caught once
    for real instead of only rehearsed by the mutation test below."""
    assert real_recipe_count(FENCEPOST_ROOT) == 113
